<x-layouts.app>
    ...
    <x-slot name="styles">
        @vite(['frontends/ayurvexa/css/home.css'])
    </x-slot>
                    ...
                    @foreach($carouselCategories as $cat)
                        <a href="{{ route('category.show', $cat) }}"
                           ...
                           style="min-width: 80px; max-width: 90px;">
                            ...
                            @endphp
                            <div class="w-16 h-16 lg:w-[72px] lg:h-[72px] rounded-full overflow-hidden border-2 border-transparent group-hover:border-primary-600 transition-all bg-[#f8f6f3] flex items-center justify-center shadow-sm">
                                @if($catImage)
                                    <img src="{{ $catImage }}"
                                         alt="{{ $cat->name }}"
    ...
    @if(isset($sections['video_testimonials']) && $sections['video_testimonials']->is_active)
        ...
        @endphp
        @if(count($vtVideos))
        <section class="py-10 lg:py-14 bg-white" x-data="vtCarousel()">
            <div class="container mx-auto px-4">
                ...
                <div class="relative">
                    {{-- Left Arrow --}}
                    <button @click="scrollLeft()" x-show="canScrollLeft" x-transition
                        class="vt-arrow vt-arrow--left">
                    ...
                    {{-- Right Arrow --}}
                    <button @click="scrollRight()" x-show="canScrollRight" x-transition
                        class="vt-arrow vt-arrow--right">
                    ...
                    {{-- Track --}}
                    <div class="vt-track" x-ref="vtTrack" @scroll.debounce.100="updateArrows()">
        ...
        <style>
            .vt-track {
                ...
                padding: 4px 16px 8px; scroll-behavior: smooth;
            }
            ...
            .vt-arrow:hover { background: var(--color-primary-600, #16a34a); color: #fff; border-color: var(--color-primary-600, #16a34a); }
            .vt-arrow--left { left: 0; }
            .vt-arrow--right { right: 0; }
            @media (max-width: 767px) {
                .vt-card { flex: 0 0 160px; }
                .vt-arrow { display: none; }
                .vt-track { gap: 10px; padding: 4px 12px 8px; }
            }
            @media (max-width: 480px) {
                .vt-card { flex: 0 0 140px; }
                .vt-track { gap: 8px; padding: 4px 10px 8px; }
            }
        </style>
        <script>
        document.addEventListener('alpine:init', function() {
            Alpine.data('vtCarousel', function() {
                return {
                    canScrollLeft: false,
                    canScrollRight: true,
                    updateArrows: function() {
                        var track = this.$refs.vtTrack;
                        if (!track) return;
                        this.canScrollLeft = track.scrollLeft > 10;
                        this.canScrollRight = track.scrollLeft < (track.scrollWidth - track.clientWidth - 10);
                    },
                    scrollLeft: function() {
                        this.$refs.vtTrack.scrollBy({ left: -430, behavior: 'smooth' });
                    },
                    ...
                    init: function() {
                        var self = this;
                        this.$nextTick(function() { self.updateArrows(); });
                    }
                };
            });
        });
