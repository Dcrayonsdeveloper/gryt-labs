<x-layouts.app>
    ...
    @push('meta')
        <meta name="description" content="Secure checkout at {{ config('app.name') }}. Fast & safe online payment.">
    ...
    <div class="bg-[#F7F8FA] min-h-screen">
        ...
        <div class="container mx-auto px-3 pb-8">
            ...
            {{-- Payment availability computed here (Cashfree not in controller) --}}
            @php
                ...
                $methodOrder = ['razorpay' => $razorpayAvailable, 'cashfree' => $cashfreeAvailable, 'cod' => $codAvailable];
                $firstMethodComputed = 'cod';
                foreach ($methodOrder as $method => $available) {
                    if ($available) { $firstMethodComputed = $method; break; }
                }
            @endphp

            <form action="{{ route('checkout.process') }}" method="POST"
                  x-data="checkoutForm('{{ $firstMethodComputed }}')"
                ...
                <div id="checkout-grid" class="space-y-4">
                    ...
                    <div id="checkout-right">
                        ...
                        <div class="bg-white rounded border border-[#E3E6E6] lg:sticky lg:top-20"
                             x-data="orderSummary()"
                             @payment-changed.window="isPrepaid = ($event.detail.method === 'razorpay' || $event.detail.method === 'cashfree')">
                            ...
                            <div class="p-3 pt-0">
                                <button type="submit" :disabled="processing"
                                    ...
                                    <span x-show="!processing">
                                        ...
                                        <template x-if="liveTotal > 0 && (paymentMethod === 'razorpay' || paymentMethod === 'cashfree')">
    ...
    <x-slot name="scripts">
        @if($razorpayAvailable || ($codAvailable && !$cashfreeAvailable))
