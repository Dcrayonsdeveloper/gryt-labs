<x-layouts.app>
    ...
    @push('meta')
        <style>body { overflow-x: clip !important; }</style>
    ...
    @php
        $certImg = $theme->get('product_certifications_image', '');
    @endphp
    <div class="container mx-auto px-4 py-4 lg:py-6">
        <!-- Product Main Section -->
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 lg:gap-6" id="pdp-main-grid">
            <!-- Image Gallery — Col 1 -->
            <div x-data="{
                 ...
                 class="" id="pdp-image-col">

                <div class="space-y-3 lg:sticky lg:top-4" id="pdp-image-inner">
            ...
            <!-- Product Info + Buy Box — Col 2 (Right) -->
            <div class="space-y-3">
                <!-- Brand -->
                @if($product->brand)
                ...
                <div class="flex items-start justify-between gap-4">
                    ...
                    <div class="flex items-center gap-2 shrink-0">
                        @if($certImg)
                        <img src="{{ asset($certImg) }}" alt="Certifications" class="h-10 lg:h-12 object-contain" loading="lazy">
                        @endif
                        <img src="{{ asset('images/usfda-registered.jpeg') }}" alt="USFDA Registered" class="h-10 lg:h-12 object-contain" loading="lazy">
                    </div>
                </div>
                ...
                @php
                    $__sliderText = $theme->get('pdp_text_slider', '');
                    $__sliderItems = $__sliderText ? array_filter(array_map('trim', explode('|', $__sliderText))) : [];
                @endphp
                ...
                <div class="flex items-start justify-between gap-3 sm:gap-4">
                    <div class="space-y-1 flex-1 min-w-0">
                        @php $__dealBadge = $theme->get('pdp_deal_badge_text', ''); @endphp
                        ...
                        @endif
                        @if($theme->get('pdp_tax_text'))
                            <p class="text-xs text-[#3a3a3a]">{{ $theme->get('pdp_tax_text') }}</p>
                        @endif
                        @if((bool) $theme->get('loyalty_enabled', true))
                            @php $__loyaltyPoints = (int) floor((float) $product->price * (int) $theme->get('loyalty_earn_rate', 1)); @endphp
                ...
                {{-- Marketplace Links (Amazon / Flipkart) --}}
                @if($product->amazon_url || $product->flipkart_url)
                <div class="flex items-center gap-3 mt-3 mb-1">
                    <span class="text-xs text-[#565959]">Also available on</span>
                    @if($product->amazon_url)
                    <a href="{{ $product->amazon_url }}" target="_blank" rel="noopener noreferrer" class="inline-flex items-center gap-1.5 px-3 py-1.5 border border-[#D5D9D9] rounded-full hover:bg-[#F7FAFA] transition-colors" title="Buy on Amazon">
                        <img src="/images/icons/amazon-icon.png" alt="Amazon" class="w-5 h-5 object-contain">
                        <span class="text-xs font-medium text-[#0F1111]">Amazon</span>
                        <svg class="w-3 h-3 text-[#565959]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14"/></svg>
                    </a>
                    @endif
                    @if($product->flipkart_url)
                    <a href="{{ $product->flipkart_url }}" target="_blank" rel="noopener noreferrer" class="inline-flex items-center gap-1.5 px-3 py-1.5 border border-[#D5D9D9] rounded-full hover:bg-[#F7FAFA] transition-colors" title="Buy on Flipkart">
                        <svg class="w-5 h-5" viewBox="0 0 24 24" fill="none"><path d="M3 4.5A1.5 1.5 0 014.5 3h3A1.5 1.5 0 019 4.5v3A1.5 1.5 0 017.5 9H6v2.5h1.5a4.5 4.5 0 014.5 4.5v1.5A1.5 1.5 0 0110.5 19h-3A1.5 1.5 0 016 17.5v-1.5a1.5 1.5 0 011.5-1.5H9V12H7.5A4.5 4.5 0 013 7.5v-3z" fill="#2874F0"/><circle cx="17" cy="5" r="2" fill="#FFE500"/><path d="M15.5 9h3a.5.5 0 01.5.5V20a1 1 0 01-1 1h-2a1 1 0 01-1-1V9.5a.5.5 0 01.5-.5z" fill="#2874F0"/></svg>
                        <span class="text-xs font-medium text-[#0F1111]">Flipkart</span>
                        <svg class="w-3 h-3 text-[#565959]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14"/></svg>
                    </a>
                    @endif
                </div>
                ...
                <!-- Stock Urgency -->
                @php $__lowStockThreshold = (int) $theme->get('low_stock_threshold', 5); @endphp
                ...
                {{-- Free Delivery Info (below stock) --}}
                @php
                    $__freeThreshold = $theme->get('free_shipping_threshold', '');
                    $__deliveryDays = (int) $theme->get('pdp_delivery_days', 0);
                    $__fastestText = $theme->get('pdp_fastest_delivery_text', '');
                @endphp
                @if($__freeThreshold && (float) $product->price >= (float) $__freeThreshold)
                <div class="text-sm mt-1">
                    <span class="text-[#007600] font-medium">FREE delivery</span>
                    @if($__deliveryDays > 0)
                        <span class="text-[#0F1111] font-medium">{{ now()->addDays($__deliveryDays)->format('D, d M') }}</span>
                    @endif
                </div>
                @if($__fastestText)
                    <div class="text-xs text-[#3a3a3a]">Or fastest delivery <span class="font-medium text-[#0F1111]">{{ $__fastestText }}</span></div>
                @endif
                @endif

                {{-- Social Proof (supports {min-max} for daily-rotating number) --}}
                @if($product->social_proof_text)
                @php
                    $__spText = $product->social_proof_text;
                    if (preg_match('/\{(\d+)-(\d+)\}/', $__spText, $__m)) {
                        $__min = (int) $__m[1];
                        $__max = (int) $__m[2];
                        $__seed = abs(crc32($product->id . now()->format('Y-m-d')));
                        $__num = $__min + ($__seed % (max(1, $__max - $__min + 1)));
                        $__spText = str_replace($__m[0], $__num, $__spText);
                    }
                @endphp
                ...
                {{-- Clinical stats carousel (per-product first, then global fallback) --}}
                @php
                    $__stats = $product->stats_carousel;
                    if (empty($__stats)) {
                        $__statsJson = $theme->get('product_stats_carousel', '');
                        $__stats = $__statsJson ? json_decode($__statsJson, true) : [];
                    }
                @endphp
                @if(!empty($__stats))
                ...
                <!-- Available Coupons -->
                @if($theme->get('show_product_coupons', '1') && isset($availableCoupons) && $availableCoupons->count())
                <div class="space-y-2">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-2">
                        ...
                        <div class="flex items-start gap-3 border border-dashed border-accent-500/60 bg-[#FFFBF2] rounded-lg p-2.5 cursor-pointer hover:border-accent-500 hover:bg-[#FFF3E0] transition-colors relative" onclick="navigator.clipboard.writeText('{{ $coupon->code }}'); let t=this.querySelector('.copy-msg'); t.style.display='block'; setTimeout(()=>t.style.display='none',1500);">
                            <div class="shrink-0 bg-[#FFF3E0] border border-dashed border-accent-500 rounded px-2 py-1 text-center min-w-[70px] relative">
                ...
                @php
                    ...
                    if (empty($benefits)) {
                        $benefits = json_decode($theme->get('product_benefits', ''), true);
                    }
                @endphp
                @if(!empty($benefits))
                ...
                @php
                    ...
                    if ($packsEnabled) {
                        ...
                        $unitLabel = $pc['unit_label'] ?? $theme->get('pack_unit_label', 'Capsules');
                        $unitsPerPack = (int) ($pc['units_per_qty'] ?? $theme->get('pack_units_per_qty', 60));
                        $monthsPerPack = (int) ($pc['months_per_qty'] ?? $theme->get('pack_months_per_qty', 1));
                        $tiersJson = $theme->get('pack_tiers', '[{"qty":1,"discount":0,"badge":null},{"qty":2,"discount":5,"badge":"MOST POPULAR"},{"qty":3,"discount":10,"badge":"BEST VALUE"}]');
                        $tiers = json_decode($tiersJson, true) ?: [];

                        // Best applicable percentage coupon per pack — drives the dynamic
                        // "Save X% + Get Extra Y% Off" banner. Excludes personalised codes
                        // (welcome/first-order, abandoned-cart, review reward) so we only
                        // surface coupons every shopper can actually redeem at this price.
                        $packCoupons = \App\Models\Coupon::where('is_active', true)
                            ...
                            ->where('code', 'NOT LIKE', 'WELCOME%')
                            ->orderByDesc('value')
                            ->get(['code', 'value', 'min_order_amount', 'max_discount']);

                        foreach ($tiers as $tier) {
                            $qty = (int) ($tier['qty'] ?? 1);
                            $discountPct = (float) ($tier['discount'] ?? 0);
                            $packMrp = (int) round($unitMrp * $qty);
                            $packPrice = (int) round($unitPrice * $qty * (1 - $discountPct / 100));
                            $savings = $packMrp - $packPrice;
                            $totalMonths = $monthsPerPack * $qty;
                            $bestCoupon = $packCoupons->first(fn($c) => (float) $c->min_order_amount <= $packPrice);

                            // Project the post-coupon final price the customer would pay
                            // if they applied this coupon at checkout. Respects max_discount cap.
                            $couponDiscount = 0.0;
                            if ($bestCoupon) {
                                $couponDiscount = $packPrice * ((float) $bestCoupon->value / 100);
                                if ($bestCoupon->max_discount && $couponDiscount > (float) $bestCoupon->max_discount) {
                                    $couponDiscount = (float) $bestCoupon->max_discount;
                                }
                            }
                            $finalWithCoupon = (int) round($packPrice - $couponDiscount);

                            $packsData[] = [
                                'qty' => $qty,
                                'label' => 'Pack of ' . $qty,
                                'desc' => ($unitsPerPack * $qty) . ' ' . $unitLabel . ' - ' . $totalMonths . ($totalMonths === 1 ? ' Month' : ' Months'),
                                'badge' => $tier['badge'] ?? null,
                                'price' => $packPrice,
                                'mrp' => $packMrp,
                                'savings' => $savings,
                                'savingsPct' => $packMrp > 0 ? (int) (($savings / $packMrp) * 100) : 0,
                                'extraOffPct' => $bestCoupon ? (int) $bestCoupon->value : 0,
                                'extraOffCode' => $bestCoupon?->code,
                                'finalWithCoupon' => $finalWithCoupon,
                            ];
                        }
                    }
                @endphp

                @if($packsEnabled && count($packsData) > 0)
                {{-- Initialize the shared pack store with PHP-computed pack data --}}
                <div x-data x-init="$store.pdpPack.init({{ \Illuminate\Support\Js::from($packsData) }}, 2)" class="mb-3">
                    <h3 class="text-base font-bold text-[#0F1111] mb-3">Choose Your Pack</h3>
                    <div class="space-y-2">
                        <template x-for="pack in $store.pdpPack.packs" :key="pack.qty">
                            <label @click="$store.pdpPack.select(pack.qty)"
                                   class="flex items-center gap-2.5 sm:gap-3 p-2.5 sm:p-3 rounded-lg border-2 cursor-pointer transition-all"
                                   :class="$store.pdpPack.selected === pack.qty ? 'border-primary-600 bg-primary-50/30' : 'border-neutral-200 hover:border-neutral-300'">
                                <input type="radio" name="pack_qty" :value="pack.qty" :checked="$store.pdpPack.selected === pack.qty" class="hidden">
                                <img src="{{ $product->primary_image_url }}" alt="{{ $product->name }}" class="w-11 h-11 sm:w-14 sm:h-14 rounded object-contain border border-neutral-100 bg-white shrink-0">
                                <div class="flex-1 min-w-0">
                                    <div class="flex items-center gap-2 flex-wrap">
                                        <span class="font-bold text-sm text-[#0F1111]" x-text="pack.label"></span>
                                        <template x-if="pack.badge">
                                            <span class="text-[9px] font-bold text-neutral-700 bg-neutral-200 px-1.5 py-0.5 rounded ml-auto" x-text="pack.badge"></span>
                                        </template>
                                    </div>
                                    <p class="text-xs text-neutral-700" x-text="pack.desc"></p>
                                    <template x-if="pack.extraOffPct > 0">
                                        <p class="text-xs font-bold text-green-700 mt-1"
                                           x-text="(pack.savingsPct > 0 ? 'Save ' + pack.savingsPct + '% + ' : '') + 'Get Extra ' + pack.extraOffPct + '% Off'"></p>
                                    </template>
                                </div>
                                <div class="text-right shrink-0">
                                    <template x-if="pack.savings > 0">
                                        <span class="text-xs text-neutral-500 line-through block" x-text="$store.pdpPack.formatPrice(pack.mrp)"></span>
                                    </template>
                                    <span class="text-lg font-bold text-[#0F1111] block" x-text="$store.pdpPack.formatPrice(pack.price)"></span>
                                </div>
                            </label>
                        </template>
                    </div>

                    {{-- Selected pack summary bar --}}
                    <div class="mt-3 flex items-center justify-between gap-3 p-3 rounded-lg bg-primary-50/50 border border-primary-100">
                        <div>
                            <p class="text-base font-bold text-[#0F1111]" x-text="$store.pdpPack.current ? $store.pdpPack.current.label : ''"></p>
                            <p class="text-xs font-semibold text-primary-600" x-text="$store.pdpPack.formatPrice($store.pdpPack.pricePerPack) + ' / Pack'"></p>
                        </div>
                        <div class="text-right">
                            <div class="flex items-baseline gap-2 justify-end">
                                <span class="text-xl font-bold text-[#0F1111]" x-text="$store.pdpPack.formatPrice($store.pdpPack.currentPrice)"></span>
                                <template x-if="$store.pdpPack.currentSavings > 0">
                                    <span class="text-sm text-neutral-500 line-through" x-text="$store.pdpPack.formatPrice($store.pdpPack.currentMrp)"></span>
                                </template>
                            </div>
                            @if($theme->get('pdp_tax_text'))
                                <p class="text-[10px] text-neutral-700">MRP {{ $theme->get('pdp_tax_text') }}</p>
                            @endif
                        </div>
                    </div>
                </div>
                ...
                {{-- Buy Box (inline) --}}
                <div x-data="quantitySelector()" class="mt-3 sm:mt-4 border-t border-[#E3E6E6] pt-3 sm:pt-4 space-y-3">
                    <!-- Quantity (hidden when pack selector controls quantity) -->
                    @if($product->stock_quantity > 0 && !($packsEnabled && count($packsData) > 0))
                    <div class="flex items-center gap-3">
                        <label class="text-sm text-[#0F1111]">Qty:</label>
                        <select x-model="quantity" class="border border-[#D5D9D9] rounded-lg bg-[#F0F2F2] text-sm py-1.5 px-3 shadow-sm focus:ring-primary-600 focus:border-link">
                    ...
                    @php $useStorePackQty = $packsEnabled && count($packsData) > 0; @endphp
                    <div class="flex gap-2 sm:gap-3" data-sticky-trigger>
                        @if($product->stock_quantity > 0)
                            <button x-data="{ adding: false, added: false }"
                                    ...
                                    :disabled="adding"
                                    class="flex-1 flex items-center justify-center gap-2 bg-accent-500 hover:bg-accent-600 text-white font-semibold py-3 px-4 rounded-lg text-sm transition-colors disabled:opacity-70 shadow-sm">
                                <span x-text="adding ? 'Adding...' : (added ? 'Added!' : 'Add to Cart')"></span>
                            </button>
                            <button id="main-buy-btn" type="button"
                                    onclick="window.__srBuyNow(event, this, {{ $product->id }})"
                                    class="flex-1 flex items-center justify-center gap-2 bg-[#FFD814] hover:bg-[#F7CA00] text-[#0F1111] font-semibold py-3 px-4 rounded-lg text-sm transition-colors shadow-sm disabled:opacity-70">
                                <span>Buy Now</span>
                            </button>
                        @else
                            <div x-data="{
                                ...
                                async submit() {
                                    ...
                                    this.error = '';
                                    try {
                                        const res = await axios.post('{{ route('product.notify-stock', $product) }}', { email: this.email });
                                        this.submitted = true;
                                        $store.toast.success(res.data.message || 'We\'ll notify you when it\'s back!');
                                    } catch (e) {
                                ...
                                <p class="text-sm font-medium text-[#B12704]">Currently out of stock</p>
                                <template x-if="!submitted">
                                    <div class="flex gap-2">
                                        <input type="email" x-model="email" placeholder="Enter your email"
                                               @keydown.enter.prevent="submit()"
                                               class="flex-1 border border-[#D5D9D9] rounded-lg text-sm py-2 px-3 focus:ring-primary-600 focus:border-link" />
                                        <button @click="submit()" :disabled="submitting"
                                                class="shrink-0 bg-accent-500 hover:bg-accent-600 text-white font-semibold py-2 px-4 rounded-lg text-sm transition-colors disabled:opacity-70">
                                            <span x-text="submitting ? 'Saving...' : 'Notify Me'"></span>
                                        </button>
                                    </div>
                                </template>
                                <template x-if="submitted">
                                    <p class="text-sm text-[#007600] font-medium py-2">We'll email you when this item is back in stock.</p>
                                </template>
                    ...
                    <!-- Secure Transaction + Payment Icons -->
                    @php $__paymentMethods = $theme->get('pdp_payment_methods', ''); @endphp
                    @if($__paymentMethods)
                    <div class="space-y-1.5">
                        <div class="flex items-center gap-1.5 text-xs">
                            <svg class="w-3.5 h-3.5 text-[#067D62]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z"/></svg>
                            <span class="text-[#067D62] font-medium">Secure transaction</span>
                        </div>
                        <div class="flex items-center gap-1.5 flex-wrap">
                            @foreach(explode(',', $__paymentMethods) as $method)
                                <span class="text-[9px] font-bold text-white bg-neutral-700 px-1.5 py-0.5 rounded-sm">{{ trim($method) }}</span>
                            @endforeach
                        </div>
                    </div>
                    @endif
                    ...
                    <!-- Sold by -->
                    <div class="text-xs text-[#3a3a3a] space-y-0.5">
                        <div>Ships from <span class="text-[#0F1111] font-medium">{{ $theme->get('store_name', config('app.name')) }}</span></div>
                        <div>Sold by <span class="text-link">{{ $product->seller?->store_name ?? $theme->get('store_name', config('app.name')) }}</span></div>
                    </div>

                    {{-- Certifications now shown next to product title --}}

                    <!-- Trust Badges (dynamic from settings) -->
                    @php
                        $__trustBadgesJson = $theme->get('pdp_trust_badges', '');
                        $__trustBadges = $__trustBadgesJson ? json_decode($__trustBadgesJson, true) : [];
                    @endphp
                    @if(!empty($__trustBadges))
                    <div class="grid grid-cols-3 sm:grid-cols-{{ min(count($__trustBadges), 4) }} gap-2 sm:gap-1.5 py-2 border-t border-[#E3E6E6]">
                        @foreach($__trustBadges as $badge)
                        <div class="flex flex-col items-center text-center">
                            <div class="w-9 h-9 bg-[#F0F8F8] rounded-full flex items-center justify-center mb-1">
                                <svg class="w-4 h-4 text-primary-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M5 13l4 4L19 7"/></svg>
                            </div>
                            <p class="text-[9px] font-medium text-[#0F1111] leading-tight">{{ $badge['title'] ?? '' }}</p>
                            @if(!empty($badge['subtitle']))
                                <p class="text-[8px] text-[#3a3a3a]">{{ $badge['subtitle'] }}</p>
                            @endif
                        </div>
                        @endforeach
                    </div>
        ...
        <section class="mt-2 sm:mt-8 border-t border-[#E3E6E6] pt-4 sm:pt-6" x-data="{ activeTab: '{{ $defaultTab }}' }">

            {{-- Tabbed navigation --}}
            <div class="flex border-b border-[#E3E6E6] gap-0 overflow-x-auto flex-nowrap min-w-0" style="-ms-overflow-style:none;scrollbar-width:none;-webkit-overflow-scrolling:touch;">
                @if($hasDescription)
                <button @click="activeTab = 'description'" :class="activeTab === 'description' ? 'border-primary-600 text-primary-600 font-semibold' : 'border-transparent text-[#3a3a3a] hover:text-[#0F1111] hover:border-[#C7C8C8]'" class="px-2 sm:px-4 py-3 text-xs sm:text-sm font-medium border-b-2 whitespace-nowrap transition-colors -mb-px shrink-0">Description</button>
                @endif
                @if($hasAttributes)
                <button @click="activeTab = 'details'" :class="activeTab === 'details' ? 'border-primary-600 text-primary-600 font-semibold' : 'border-transparent text-[#3a3a3a] hover:text-[#0F1111] hover:border-[#C7C8C8]'" class="px-2 sm:px-4 py-3 text-xs sm:text-sm font-medium border-b-2 whitespace-nowrap transition-colors -mb-px shrink-0">Details</button>
                @endif
                @if($hasInfo)
                <button @click="activeTab = 'information'" :class="activeTab === 'information' ? 'border-primary-600 text-primary-600 font-semibold' : 'border-transparent text-[#3a3a3a] hover:text-[#0F1111] hover:border-[#C7C8C8]'" class="px-2 sm:px-4 py-3 text-xs sm:text-sm font-medium border-b-2 whitespace-nowrap transition-colors -mb-px shrink-0">Information</button>
                @endif
                <button @click="activeTab = 'faq'" :class="activeTab === 'faq' ? 'border-primary-600 text-primary-600 font-semibold' : 'border-transparent text-[#3a3a3a] hover:text-[#0F1111] hover:border-[#C7C8C8]'" class="px-2 sm:px-4 py-3 text-xs sm:text-sm font-medium border-b-2 whitespace-nowrap transition-colors -mb-px shrink-0">FAQ</button>
            </div>
            <div x-show="activeTab === 'faq'" x-cloak class="pt-6"><x-faq-section :product="$product" class="!py-0 !bg-transparent" /></div>
            ...
            @if($hasAttributes)
            <div x-show="activeTab === 'details'" x-cloak class="pt-6">
            ...
            @if($hasInfo)
            <div x-show="activeTab === 'information'" x-cloak class="pt-6">
        ...
        @if(isset($compareProducts) && $compareProducts->count() && $theme->get('show_product_compare', '1') === '1')
                ...
                <div class="flex gap-4 overflow-x-auto pb-2" style="-ms-overflow-style:none;scrollbar-width:none;">
                    @foreach($pdpTestimonialVideos as $vid)
                        <div class="relative rounded-xl overflow-hidden bg-black shadow-sm ring-1 ring-neutral-200 shrink-0" style="width:200px;aspect-ratio:9/16;">
                            <video
                                src="{{ Str::startsWith($vid, 'http') ? $vid : (Str::contains($vid, '/') ? asset($vid) : asset('images/' . $vid)) }}"
                ...
                <div class="lg:col-span-8">
                    ...
                    @if($displayReviews->count())
                      <div x-data="{
                          page: 2,
                          loading: false,
                          hasMore: {{ $product->review_count > 10 ? 'true' : 'false' }},
                          loadMore() {
                              this.loading = true;
                              fetch('/products/{{ $product->slug }}/reviews?page=' + this.page)
                                  .then(r => r.json())
                                  .then(data => {
                                      let container = this.$refs.reviewsList;
                                      data.data.forEach(review => {
                                          let div = document.createElement('div');
                                          div.className = 'border-b border-[#E3E6E6] pb-5';
                                          let stars = '';
                                          for (let i = 1; i <= 5; i++) {
                                              stars += `<svg class=\'w-4 h-4 ${i <= review.rating ? 'text-primary-600' : 'text-[#767676]'}' fill='currentColor' viewBox='0 0 20 20'><path d='M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z'/></svg>`;
                                          }
                                          let name = review.guest_name || (review.user ? review.user.first_name + ' ' + (review.user.last_name || '') : 'Anonymous');
                                          let initial = name.charAt(0).toUpperCase();
                                          let verified = review.is_verified_purchase ? '<span class=\'text-xs font-bold text-[#C45500]\'>Verified Purchase</span>' : '';
                                          let timeAgo = this.formatTimeAgo(new Date(review.created_at));
                                          let pros = typeof review.pros === 'string' ? JSON.parse(review.pros || '[]') : (review.pros || []);
                                          let prosHtml = '';
                                          if (pros.length) {
                                              prosHtml = '<div class=\'mt-2 flex flex-wrap gap-1.5\'>' + pros.map(p => `<span class='inline-flex items-center gap-1 text-xs bg-[#F0FFF4] text-[#007600] px-2 py-0.5 rounded-full border border-[#C6F6D5]'><svg class='w-3 h-3' fill='none' stroke='currentColor' viewBox='0 0 24 24'><path stroke-linecap='round' stroke-linejoin='round' stroke-width='2' d='M5 13l4 4L19 7'/></svg>${p}</span>`).join('') + '</div>';
                                          }
                                          div.innerHTML = `
                                              <div class='flex items-center gap-2 mb-1.5'>
                                                  <div class='w-8 h-8 bg-[#F0F2F2] rounded-full flex items-center justify-center'><span class='text-xs font-medium text-[#3a3a3a]'>${initial}</span></div>
                                                  <span class='text-sm text-[#0F1111]'>${name}</span>
                                              </div>
                                              <div class='flex items-center gap-2 mb-1'><div class='flex'>${stars}</div>${review.title ? `<span class='text-sm font-bold text-[#0F1111]'>${review.title}</span>` : ''}</div>
                                              <div class='flex items-center gap-2 mb-2'><span class='text-xs text-[#3a3a3a]'>Reviewed ${timeAgo}</span>${verified}</div>
                                              <p class='text-sm text-[#0F1111] leading-relaxed'>${review.content}</p>
                                              ${prosHtml}
                                              ${review.helpful_count > 0 ? `<div class='mt-3 flex items-center gap-3'><span class='text-xs text-[#3a3a3a]'>${review.helpful_count} ${review.helpful_count == 1 ? 'person' : 'people'} found this helpful</span></div>` : ''}
                                          `;
                          ...
                          formatTimeAgo(date) {
                              ...
                              if (days > 30) { let months = Math.floor(days / 30); return months + (months === 1 ? ' month ago' : ' months ago'); }
                        ...
                        <div class="space-y-6" x-ref="reviewsList">
                            @foreach($displayReviews as $review)
                                <div class="border-b border-[#E3E6E6] pb-5">
                                    ...
                                    <!-- Date + Verified -->
                                    <div class="flex items-center gap-2 mb-2">
                                        <span class="text-xs text-[#3a3a3a]">Reviewed {{ $review->created_at->diffForHumans() }}</span>
                                    ...
                                    <div class="mt-3 flex items-center gap-3">
                                        <span class="text-xs text-[#3a3a3a]">{{ $review->helpful_count }} {{ $review->helpful_count == 1 ? 'person' : 'people' }} found this helpful</span>
                                    </div>
                                    @endif
                        ...
                        @if($product->review_count > 10)
                            <div class="mt-4">
                                <button x-show="hasMore" @click="loadMore()" :disabled="loading" class="text-sm font-medium text-primary-600 hover:text-primary-700 hover:underline transition-colors disabled:opacity-50 cursor-pointer">
                                    <span x-show="!loading">See more reviews</span>
    ...
    @if($product->stock_quantity > 0)
        ...
        (function() {
            ...
            @php $usePackQtySticky = $packsEnabled && count($packsData ?? []) > 0; @endphp
            var usePack = {{ $usePackQtySticky ? 'true' : 'false' }};

            // Get current quantity from the main page selector or pack store
            function getQty() {
                if (usePack && window.Alpine && Alpine.store('pdpPack')) {
                    return Alpine.store('pdpPack').currentQty || 1;
                }
                var sel = document.querySelector('select[x-model="quantity"]');
                return sel ? parseInt(sel.value) || 1 : 1;
            }
            function setQty(val) {
                if (usePack) return; // Pack qty controlled by pack selector
                var sel = document.querySelector('select[x-model="quantity"]');
            ...
            // Wire up Buy Now (Shiprocket Checkout)
            document.getElementById('sticky-buy-btn').addEventListener('click', function(e) {
                var btn = document.getElementById('sticky-buy-btn');
                window.__srBuyNow(e, btn, {{ $product->id }});
            });
