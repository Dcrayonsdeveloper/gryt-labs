<?php $announcement = $theme->get('announcement_text', ''); ?>
...
<header id="main-header" x-data="{ visible: true, lastScroll: 0 }"
    ...
    <div class="text-neutral-900 py-1.5 text-[11px] sm:text-xs font-medium tracking-wide" style="background-color:#B8DC24;">
        <div class="container mx-auto px-4 flex items-center justify-between">
            ...
            <span class="sm:hidden flex-1 text-center"><?php echo e($announcement); ?></span>
            <div class="hidden sm:flex items-center gap-3">
                <?php
                ...
                ?>
                <?php if($headerPhone): ?>
                <a href="tel:<?php echo e(preg_replace('/[^0-9+]/', '', (string) $headerPhone)); ?>" class="inline-flex items-center gap-1 text-neutral-800 hover:text-black transition">
        ...
        <div class="flex items-center justify-between h-14 lg:h-16">
            ...
            <div class="flex items-center gap-3 lg:gap-0 flex-1">
                ...
                <?php if($hasCustomNav): ?>
                    ...
                    <a href="<?php echo e(url('/')); ?>" class="flex items-center shrink-0">
                        <img src="<?php echo e(asset($theme->get('store_logo', 'images/logo.png'))); ?>" alt="<?php echo e($theme->get('store_name', config('app.name'))); ?>" style="height: <?php echo e($theme->get('logo_height', '48')); ?>px; width: auto; max-width: 180px;">
                    </a>
                <?php else: ?>
            ...
            <?php if($hasCustomNav): ?>
                <nav class="hidden lg:flex items-center gap-1 shrink-0">
                    <?php $__currentLoopData = $headerNavItems; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $navItem): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <?php if($navItem->children->isNotEmpty()): ?>
                            <div class="relative" x-data="{ open: false }" @click.outside="open = false">
                                ...
                                <div x-show="open" x-cloak x-transition:enter="transition ease-out duration-150" x-transition:enter-start="opacity-0 -translate-y-1" x-transition:enter-end="opacity-100 translate-y-0" x-transition:leave="transition ease-in duration-100" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0"
                                    ...
                                    <div class="bg-white border border-neutral-200 rounded-lg shadow-lg py-1 min-w-[200px]">
                                        <?php $__currentLoopData = $navItem->children; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $child): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                            <a href="<?php echo e($child->url); ?>" class="block px-4 py-2.5 text-sm text-neutral-700 hover:bg-primary-50 hover:text-primary-700 transition-colors"><?php echo e($child->label); ?></a>
                                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                                    </div>
                                </div>
                            </div>
                        <?php else: ?>
                            <a href="<?php echo e($navItem->url); ?>" class="px-3 py-2 text-[13px] text-[#222] hover:text-primary-600 font-medium transition-colors tracking-wide uppercase"><?php echo e($navItem->label); ?></a>
                        <?php endif; ?>
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                </nav>
            <?php else: ?>
                <a href="<?php echo e(url('/')); ?>" class="flex items-center shrink-0 mx-4">
                    <img src="<?php echo e(asset($theme->get('store_logo', 'images/logo.png'))); ?>" alt="<?php echo e($theme->get('store_name', config('app.name'))); ?>" style="height: <?php echo e($theme->get('logo_height', '48')); ?>px; width: auto; max-width: 180px;">
                </a>
            <?php endif; ?>
            ...
            <div class="flex items-center gap-1 lg:gap-0 flex-1 justify-end">
                ...
                <nav class="hidden lg:flex items-center gap-1 mr-2">
                    <?php if(config('app.wholesale_enabled')): ?>
                        <a href="<?php echo e(route('wholesale')); ?>" class="px-3 py-2 text-[13px] text-[#222] hover:text-primary-600 font-medium transition-colors tracking-wide uppercase">Wholesale</a>
                    <?php endif; ?>
                </nav>

                <!-- Mobile search icon (shown below sm, links to search page) -->
                <a href="<?php echo e(route('search')); ?>" class="sm:hidden p-2 text-neutral-600 hover:text-primary-600 transition-colors" aria-label="Search">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <!-- Wishlist -->
                <a href="<?php echo e(route('wishlist')); ?>" class="relative p-2 text-neutral-600 hover:text-primary-600 transition-colors hidden sm:flex" aria-label="Wishlist">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="relative hidden lg:block" x-data="dropdown()">
                    ...
                    <div x-cloak x-show="open" x-transition @click.outside="close()" class="absolute right-0 mt-1 w-48 bg-white border border-neutral-200 rounded-lg shadow-dropdown z-50 overflow-hidden">
                        <?php if(auth()->guard()->guest()): ?>
                            <a href="<?php echo e(route('login')); ?>" class="block px-4 py-2 text-sm text-neutral-700 hover:text-primary-600">Login</a>
                            <a href="<?php echo e(route('register')); ?>" class="block px-4 py-2 text-sm text-neutral-700 hover:text-primary-600">Register</a>
                        <?php else: ?>
                            <div class="px-4 py-2 border-b border-neutral-100">
                                <div class="text-sm font-medium text-neutral-900"><?php echo e(auth()->user()->full_name); ?></div>
                                <div class="text-xs text-neutral-600"><?php echo e(auth()->user()->email); ?></div>
                            </div>
                            <a href="<?php echo e(route('account.dashboard')); ?>" class="block px-4 py-2 text-sm text-neutral-700 hover:text-primary-600">Dashboard</a>
                            <a href="<?php echo e(route('account.orders.index')); ?>" class="block px-4 py-2 text-sm text-neutral-700 hover:text-primary-600">My Orders</a>
                            <a href="<?php echo e(route('account.profile')); ?>" class="block px-4 py-2 text-sm text-neutral-700 hover:text-primary-600">Profile Settings</a>
                            <?php if(auth()->user()->deliveryPartner): ?>
                                <a href="<?php echo e(route('delivery.login')); ?>" class="block px-4 py-2 text-sm text-primary-600 hover:text-primary-700 font-medium">Delivery Panel</a>
                            <?php else: ?>
                                <a href="<?php echo e(route('account.become-delivery-partner')); ?>" class="block px-4 py-2 text-sm text-primary-600 hover:text-primary-700 font-medium">Become a Delivery Partner</a>
                            <?php endif; ?>
                ...
                <!-- Cart -->
                <a href="<?php echo e(route('cart.index')); ?>" class="relative p-2 text-neutral-700 hover:text-primary-600 transition-colors" aria-label="Cart">
...
<div id="header-spacer"
     class="<?php echo e($announcement ? 'h-[86px] lg:h-[94px]' : 'h-14 lg:h-16'); ?>"
...
<?php /**PATH D:\projects\grytlabs345\frontends/default/views/partials/header.blade.php ENDPATH**/ ?>