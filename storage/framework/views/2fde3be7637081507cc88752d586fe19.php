...
<?php $component->withAttributes([]); ?>
     <?php $__env->slot('title', null, []); ?> Shopping Cart - <?php echo e(config('app.name')); ?> <?php $__env->endSlot(); ?>

    <?php $__env->startPush('meta'); ?>
        <meta name="description" content="Review your shopping cart and proceed to checkout at <?php echo e($theme->get('store_name', config('app.name'))); ?>.">
...
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.breadcrumb','data' => ['items' => [['label' => 'Shopping Cart', 'url' => null]]]] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
...
<?php $attributes = $attributes->except(\Illuminate\View\AnonymousComponent::ignoredParameterNames()); ?>
<?php endif; ?>
<?php $component->withAttributes(['items' => \Illuminate\View\Compilers\BladeCompiler::sanitizeComponentAttribute([['label' => 'Shopping Cart', 'url' => null]])]); ?>
...
<?php endif; ?>
        ...
        <div class="container mx-auto px-4 pb-10">
                        ...
                        <?php for($i = 0; $i < 2; $i++): ?>
                            <div class="bg-white rounded-lg border border-neutral-100 p-3 sm:p-4">
                                <div class="flex gap-3">
                                    <div class="w-15 h-15 bg-neutral-200 rounded shrink-0"></div>
            ...
            <template x-if="items.length > 0">
                <div>
                    ...
                    <div class="flex items-center justify-between mb-4">
                        ...
                        <a href="<?php echo e(route('products.index')); ?>" class="text-sm text-primary-600 hover:text-primary-700 font-medium">
                            Continue Shopping
                        </a>
                    </div>
                    ...
                    <div class="flex flex-col lg:flex-row lg:items-start gap-5">
                        ...
                        <div class="flex-1 min-w-0 space-y-2.5">
                            <template x-for="item in items" :key="item.id">
                                <div class="bg-white rounded-lg border border-neutral-100 p-3 sm:p-4">
                                    <div class="flex gap-3">
                                        <!-- Product Image -->
                                        <a :href="item.product_url" class="shrink-0 self-start block w-15 h-15">
                                            <img :src="item.image" :alt="item.name"
                                                 class="rounded border border-neutral-100 bg-neutral-50 h-15 w-auto max-w-15 object-contain">
                                        </a>
                        ...
                        <div class="lg:w-85 shrink-0 self-stretch">
                            ...
                            <?php $freeShipThreshold = (float) $theme->get('free_shipping_threshold', 399); ?>
                            <template x-if="subtotal < <?php echo e($freeShipThreshold); ?>">
                                <div class="bg-amber-50 border border-amber-200 rounded-md px-3.5 py-2.5 mb-3">
                                    <p class="text-xs font-semibold text-amber-700 mb-1.5">
                                        Add <span x-text="fp(<?php echo e($freeShipThreshold); ?> - subtotal)"></span> more for FREE shipping!
                                    </p>
                                    <div class="bg-amber-200 rounded h-1.5 overflow-hidden">
                                        <div class="bg-primary-600 h-full rounded transition-all duration-300" :style="'width:' + Math.min(100, (subtotal / <?php echo e($freeShipThreshold); ?>) * 100) + '%'"></div>
                                    </div>
                                </div>
                            </template>
                            <template x-if="subtotal >= <?php echo e($freeShipThreshold); ?>">
                                <div class="bg-green-50 rounded-md px-3.5 py-2 mb-3">
                                    <p class="text-xs font-semibold text-green-700">&#10003; You qualify for FREE shipping!</p>
                                </div>
                            </template>
                            ...
                            <div class="bg-white rounded-lg border border-neutral-100 sticky top-20 flex flex-col">
                                        ...
                                        <div>
                                            <form @submit.prevent="applyCoupon()" class="flex items-stretch">
                                                <input type="text" x-model="couponCode" placeholder="Enter coupon code"
                                                       class="flex-1 min-w-0 text-[13px] border border-neutral-200 border-r-0 rounded-l-lg px-3 py-2.5 focus:border-primary-400 uppercase placeholder:normal-case placeholder:text-neutral-600" required>
                                                <button type="submit"
                                ...
                                <div class="p-4 pt-0">
                                    <?php if(\App\Models\Setting::get('shiprocket_checkout_enabled', false)): ?>
                                        <button onclick="window.__srCartCheckout ? __srCartCheckout(this) : (window.location.href='<?php echo e(route('checkout.index')); ?>')"
                                           class="block w-full py-2 bg-primary-600 hover:bg-primary-700 text-white text-sm font-semibold text-center rounded-lg transition-colors shadow-sm disabled:opacity-70">
                ...
                <div class="mt-8">
                    ...
                    <div class="flex gap-3 overflow-x-auto pb-3 -mx-4 px-4 snap-x snap-mandatory scrollbar-hide">
                        <?php $__currentLoopData = $recommendations; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $rec): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <div class="shrink-0 w-40 sm:w-[180px] snap-start bg-white rounded-xl border border-neutral-100 overflow-hidden group">
                                <a href="<?php echo e($rec['url']); ?>" class="block">
                                    <div class="aspect-square bg-neutral-50 overflow-hidden relative">
                                        <img src="<?php echo e($rec['image']); ?>" alt="<?php echo e($rec['name']); ?>"
                                             class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300"
                                             loading="lazy"
                                             onerror="this.src='<?php echo e(asset('images/placeholder-product.svg')); ?>'">
                                        <?php if($rec['has_discount'] && $rec['discount_pct'] > 0): ?>
                                            <span class="absolute top-2 left-2 bg-accent-500 text-white text-[10px] font-bold px-1.5 py-0.5 rounded"><?php echo e($rec['discount_pct']); ?>% Off</span>
                                        <?php endif; ?>
                                    </div>
                                </a>
                                <div class="p-2.5">
                                    <?php if($rec['brand']): ?>
                                        <p class="text-[10px] text-neutral-700 uppercase tracking-wide mb-0.5"><?php echo e($rec['brand']); ?></p>
                                    <?php endif; ?>
                                    <a href="<?php echo e($rec['url']); ?>" class="block">
                                        <h3 class="text-xs font-medium text-neutral-800 line-clamp-2 leading-snug mb-1.5 group-hover:text-primary-600"><?php echo e($rec['name']); ?></h3>
                                    </a>
                                    <?php if($rec['rating'] > 0): ?>
                                        <div class="flex items-center gap-1 mb-1">
                                            <span class="inline-flex items-center gap-0.5 bg-[#C1539C] text-white text-[10px] font-bold px-1 py-0.5 rounded-sm">
                                                <?php echo e(number_format($rec['rating'], 1)); ?>
                                    ...
                                    <div class="flex items-baseline gap-1 flex-wrap mb-2">
                                        <span class="text-sm font-bold text-neutral-900"><?php echo format_price($rec['price']); ?></span>
                                        <?php if($rec['has_discount']): ?>
                                            <span class="text-[10px] text-neutral-700 line-through"><?php echo format_price($rec['mrp']); ?></span>
                                        <?php endif; ?>
                                    </div>
                                    <button @click="$store.cart.add(<?php echo e($rec['id']); ?>)"
                                            class="w-full py-2 text-[11px] font-semibold text-white bg-primary-600 rounded-md hover:bg-primary-700 transition-colors">
                                        Add to Bag
                                    </button>
                                </div>
                            </div>
            ...
            <template x-if="items.length === 0">
                ...
                <div class="flex flex-col items-center justify-center bg-white rounded-xl border border-neutral-100 py-20 px-6 mt-4">
                    <div class="w-24 h-24 mb-6 bg-primary-600/5 rounded-full flex items-center justify-center">
                        <svg class="w-12 h-12 text-primary-600/50" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <a href="<?php echo e(route('products.index')); ?>"
                       class="inline-flex items-center gap-2 bg-accent-500 hover:bg-accent-600 text-white text-sm font-semibold px-10 py-2 rounded-lg transition-colors shadow-md shadow-accent-500/20">
    ...
    <?php
        $cartItems = $cart->items->map(function ($item) {
            return [
                ...
                'product_url' => route('products.show', $item->product),
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            var cartItems = <?php echo json_encode($ga4CartItems, JSON_UNESCAPED_UNICODE); ?>;
...
<?php endif; ?>
<?php /**PATH D:\projects\grytlabs345\frontends/default/views/cart/index.blade.php ENDPATH**/ ?>