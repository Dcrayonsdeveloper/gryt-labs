<form action="<?php echo e(route('products.index')); ?>" method="GET" class="space-y-4">
                    ...
                    <?php $__currentLoopData = $categories; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $category): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <label class="flex items-center gap-2.5 cursor-pointer group py-0.5">
                            <input type="radio" name="category" value="<?php echo e($category->slug); ?>"
                                   ...
                                   class="w-3.5 h-3.5 border-neutral-300 text-primary-600 focus:ring-primary-600 focus:ring-offset-0"
                                   onchange="this.form.submit()">
                            <span class="text-sm text-neutral-600 group-hover:text-neutral-900 transition-colors"><?php echo e($category->name); ?></span>
                        </label>
            ...
            <div x-show="open" x-collapse>
                <div class="space-y-1.5 max-h-52 overflow-y-auto scrollbar-thin pt-1 pb-2">
                    <?php $__currentLoopData = $subcategories; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $sub): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <label class="flex items-center gap-2.5 cursor-pointer group py-0.5">
                            <input type="checkbox" name="subcategory[]" value="<?php echo e($sub->slug); ?>"
                                   <?php echo e(in_array($sub->slug, (array) request('subcategory')) ? 'checked' : ''); ?>

                                   onchange="this.form.submit()"
                                   class="w-3.5 h-3.5 rounded border-neutral-300 text-primary-600 focus:ring-primary-600 focus:ring-offset-0">
                            <span class="text-sm text-neutral-600 group-hover:text-neutral-900 transition-colors"><?php echo e($sub->name); ?></span>
                        </label>
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                </div>
            </div>
            ...
            <div class="flex items-center gap-2 pt-1 pb-2">
                <div class="relative flex-1">
                    ...
                    <input type="number" name="min_price" value="<?php echo e(request('min_price')); ?>"
                           ...
                           placeholder="Min" class="w-full pl-6 pr-2 py-2 text-sm border border-neutral-200 rounded-lg focus:outline-none focus:border-primary-600 bg-neutral-50">
                </div>
                ...
                <div class="relative flex-1">
                    ...
                    <input type="number" name="max_price" value="<?php echo e(request('max_price')); ?>"
                           onkeydown="if(event.key==='Enter'){event.preventDefault();this.form.submit();}"
                           placeholder="Max" class="w-full pl-6 pr-2 py-2 text-sm border border-neutral-200 rounded-lg focus:outline-none focus:border-primary-600 bg-neutral-50">
                </div>
            </div>
        </div>
    </div>
                ...
                <?php for($i = 4; $i >= 1; $i--): ?>
                    <label class="flex items-center gap-2.5 cursor-pointer group py-0.5">
                        ...
                        <span class="flex items-center gap-0.5">
                            <?php for($j = 1; $j <= 5; $j++): ?>
                                <svg class="w-3.5 h-3.5 <?php echo e($j <= $i ? 'text-amber-400' : 'text-neutral-200'); ?>" fill="currentColor" viewBox="0 0 20 20">
    ...
    <div class="flex gap-2 pt-2">
        <button type="submit" class="flex-1 py-2.5 bg-accent-500 hover:bg-accent-600 text-white text-sm font-semibold rounded-lg transition-colors">
            Apply
        </button>
        <a href="<?php echo e(route('products.index')); ?>" class="flex-1 py-2.5 text-center text-sm font-medium text-neutral-600 rounded-lg hover:bg-neutral-50 border border-neutral-200 transition-colors">
            Reset
        </a>
    </div>
</form>
<?php /**PATH D:\projects\grytlabs345\resources\views/products/partials/filters.blade.php ENDPATH**/ ?>