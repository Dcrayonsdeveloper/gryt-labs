    ...
    <div class="bg-primary-800 text-white py-1.5 text-[11px] sm:text-xs font-medium tracking-wide">
        <div class="container mx-auto px-4 flex items-center justify-between">
            ...
            <div class="hidden sm:flex items-center gap-3">
                ...
                @endphp
                @if($headerPhone)
                <a href="tel:{{ $headerPhone }}" class="inline-flex items-center gap-1 text-white/90 hover:text-white transition">
                    <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z"/></svg>
                    {{ $headerPhone }}
                </a>
                @endif
                @if($headerEmail)
                <a href="mailto:{{ $headerEmail }}" class="inline-flex items-center gap-1 text-white/90 hover:text-white transition">
                    <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/></svg>
                    {{ $headerEmail }}
                </a>
                @endif
                <span class="text-white/30">|</span>
                <a href="{{ url('/about-us') }}" class="text-white/90 hover:text-white transition">About Us</a>
                <a href="{{ url('/contact') }}" class="text-white/90 hover:text-white transition">Contact</a>
            </div>
        </div>
    </div>
    @endif
            ...
            <div class="flex items-center gap-3 lg:gap-0 flex-1">
                ...
                @if($hasCustomNav)
                    ...
                    <a href="{{ url('/') }}" class="flex items-center shrink-0">
                        <img src="{{ asset($theme->get('store_logo', 'images/logo.png')) }}" alt="{{ $theme->get('store_name', config('app.name')) }}" style="height: {{ $theme->get('logo_height', '48') }}px; width: auto; max-width: {{ $theme->get('logo_max_width', '180') }}px;">
                    </a>
                @else
                    <nav class="hidden lg:flex items-center gap-1">
                        ...
                        <a href="{{ route('offers') }}" class="px-3 py-2 text-[13px] text-accent-500 hover:text-accent-600 font-semibold transition-colors tracking-wide uppercase">Sale</a>
                    </nav>
                @endif
            </div>
                        ...
                        @if($navItem->children->isNotEmpty())
                            <div class="relative" x-data="{ open: false }" @click.outside="open = false">
                                ...
                                <div x-show="open" x-cloak x-transition:enter="transition ease-out duration-150" x-transition:enter-start="opacity-0 -translate-y-1" x-transition:enter-end="opacity-100 translate-y-0" x-transition:leave="transition ease-in duration-100" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0"
                                    ...
                                    <div class="bg-white border border-neutral-200 rounded-lg shadow-lg py-1 min-w-50">
                                        @foreach($navItem->children as $child)
                                            <a href="{{ $child->url }}" class="block px-4 py-2.5 text-sm text-neutral-700 hover:bg-neutral-100 hover:text-neutral-900 transition-colors">{{ $child->label }}</a>
                                        @endforeach
                                    </div>
                                </div>
                            </div>
                                ...
                                <template x-for="(result, idx) in results" :key="result.type + '-' + result.id + '-' + idx">
                                    <li>
                                        <a :href="result.url" class="flex items-center gap-2.5 px-3 py-2 hover:bg-neutral-50 transition-colors">
                                            <img :src="result.image || '/images/placeholder.png'" :alt="result.name" class="w-8 h-8 object-cover rounded bg-neutral-100" width="32" height="32" decoding="async">
