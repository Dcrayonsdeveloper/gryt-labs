    ...
    <div x-show="open"
        ...
        <div class="flex items-center justify-between px-4 py-3 border-b border-neutral-100 shrink-0">
            <a href="{{ url('/') }}" class="flex items-center">
                <img src="{{ asset($theme->get('store_logo', 'images/logo.png')) }}" alt="{{ $theme->get('store_name', config('app.name')) }}" class="h-[29px] w-auto">
            </a>
            ...
            @else
                <div class="flex items-center gap-3">
                    <div class="w-10 h-10 bg-primary-600/10 rounded-full flex items-center justify-center shrink-0">
                        ...
                        @else
                            <span class="text-sm font-semibold text-primary-600">{{ substr(auth()->user()->first_name, 0, 1) }}</span>
                        @endif
                    </div>
                    <div class="min-w-0">
        ...
        <nav class="flex-1 overflow-y-auto">
            <div class="py-2">
                ...
                @endphp
                @if($mobileNavItems->isNotEmpty())
                    @foreach($mobileNavItems as $mNavItem)
                        @if($mNavItem->children->isNotEmpty())
                            <div x-data="{ open: false }">
                                <button @click="open = !open" class="flex items-center justify-between w-full px-4 py-3 text-sm text-neutral-700 hover:bg-neutral-50">
                                    <span>{{ $mNavItem->label }}</span>
                                    <svg class="w-4 h-4 transition-transform" :class="open ? 'rotate-180' : ''" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/></svg>
                                </button>
                                <div x-show="open" x-collapse class="bg-neutral-50">
                                    @foreach($mNavItem->children as $mChild)
                                        <a href="{{ $mChild->url }}" class="block pl-8 pr-4 py-2.5 text-sm text-neutral-600 hover:text-primary-600">{{ $mChild->label }}</a>
                                    @endforeach
                                </div>
                            </div>
                        @else
                            @php $isMConsultBtn = \Illuminate\Support\Str::contains(strtolower($mNavItem->label), 'consultation'); @endphp
                            @if($isMConsultBtn)
                                <a href="{{ $mNavItem->url }}" class="mx-4 my-2 flex items-center justify-center gap-2 px-4 py-3 bg-linear-to-r from-accent-500 to-accent-600 hover:from-accent-600 hover:to-accent-700 text-white text-sm font-bold rounded-full shadow-md shadow-accent-500/25 transition-all">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z"/></svg>
                                    {{ $mNavItem->label }}
                                </a>
                            @else
                                <a href="{{ $mNavItem->url }}" class="flex items-center gap-3 px-4 py-3 text-sm text-neutral-700 hover:bg-neutral-50">{{ $mNavItem->label }}</a>
                            @endif
                        @endif
                    @endforeach
                @else
                    <a href="{{ url('/') }}" class="flex items-center gap-3 px-4 py-3 text-sm text-neutral-700 hover:bg-neutral-50">Home</a>
                    <a href="{{ route('new-arrivals') }}" class="flex items-center gap-3 px-4 py-3 text-sm text-neutral-700 hover:bg-neutral-50">New Arrivals</a>
                    <a href="{{ route('bestsellers') }}" class="flex items-center gap-3 px-4 py-3 text-sm text-neutral-700 hover:bg-neutral-50">Bestsellers</a>
                    <a href="{{ route('offers') }}" class="flex items-center gap-3 px-4 py-3 text-sm text-error-600 hover:bg-error-50/50 font-medium">Deals & Offers</a>
                @endif
                ...
                <div class="mt-2 pt-2 border-t border-neutral-100">
                            ...
                            <div x-data="{ expanded: false }">
                                ...
                                <div x-show="expanded" x-collapse>
                                    <div class="bg-neutral-50/50 py-1">
                                        <a href="{{ route('category.show', $cat) }}" class="block pl-8 pr-4 py-2 text-xs font-medium text-primary-600 hover:bg-neutral-100/50">
                                            View All {{ $cat->name }}
                                        </a>
                                        @foreach($cat->children as $child)
                                            <a href="{{ route('category.show', $child) }}" class="block pl-8 pr-4 py-2 text-sm text-neutral-600 hover:text-primary-600 hover:bg-neutral-100/50">
                                                {{ $child->name }}
                                            </a>
                                        @endforeach
                                    </div>
                    ...
                    <a href="{{ route('categories.index') }}" class="flex items-center gap-2 px-4 py-3 text-sm text-primary-600 hover:bg-primary-600/5 font-medium">
                ...
                @auth
                    <div class="mt-2 pt-2 border-t border-neutral-100">
                        ...
                        @if(auth()->user()->deliveryPartner)
                            <a href="{{ route('delivery.login') }}" class="flex items-center gap-3 px-4 py-3 text-sm text-primary-600 hover:bg-primary-600/5 font-medium">
                                <svg class="w-5 h-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        @else
                            <a href="{{ route('account.become-delivery-partner') }}" class="flex items-center gap-3 px-4 py-3 text-sm text-primary-600 hover:bg-primary-600/5 font-medium">
                                <svg class="w-5 h-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M13 16V6a1 1 0 00-1-1H4a1 1 0 00-1 1v10a1 1 0 001 1h1m8-1a1 1 0 01-1 1H9m4-1V8a1 1 0 011-1h2.586a1 1 0 01.707.293l3.414 3.414a1 1 0 01.293.707V16a1 1 0 01-1 1h-1m-6-1a1 1 0 001 1h1M5 17a2 2 0 104 0m-4 0a2 2 0 114 0m6 0a2 2 0 104 0m-4 0a2 2 0 114 0"/>
                                </svg>
