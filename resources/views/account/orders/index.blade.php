                ...
                <div class="flex-1">
                        ...
                        @foreach($statuses as $value => $label)
                            <a href="{{ route('account.orders.index', $value ? ['status' => $value] : []) }}"
                               class="shrink-0 px-3.5 py-1.5 rounded-full text-xs font-semibold transition-colors
                                      {{ $currentStatus === $value ? 'bg-accent-500 text-white' : 'bg-white border border-neutral-200 text-neutral-600 hover:border-neutral-300 hover:text-neutral-800' }}">
                                {{ $label }}
                            </a>
                        @endforeach
                    </div>
                        ...
                        <div class="bg-white rounded-xl border border-neutral-200 mb-3 overflow-hidden hover:shadow-sm transition-shadow">
                            ...
                            <div class="px-4 py-3 flex flex-wrap items-center justify-between gap-3 border-b border-neutral-100">
                                <div class="flex items-center gap-3">
                                    <a href="{{ route('account.orders.show', $order) }}" class="text-sm font-bold text-neutral-900 hover:text-primary-600 transition-colors">
                                        {{ $order->order_number }}
                                    </a>
                                ...
                                <div class="flex items-center gap-3 text-xs text-neutral-600">
                                    ...
                                    @php
                                        ...
                                        $pmPaid = (float) ($order->paid_amount ?? 0);
                                        $pmBalance = max(0, (float) $order->total - $pmPaid);
                                        $pmIsPartial = $pmBalance > 0 && $pmPaid > 0;
                                    @endphp
                                    @if($pmIsPartial)
                                        <span class="text-[10px] font-semibold px-1.5 py-0.5 rounded-full bg-amber-50 text-amber-700 border border-amber-200">Partial Pay</span>
                                    @elseif($order->payment_status === 'paid')
                                        <span class="text-[10px] font-semibold px-1.5 py-0.5 rounded-full bg-emerald-50 text-emerald-700 border border-emerald-200">Paid</span>
                                    @elseif($pmMethod === 'cod')
                                        <span class="text-[10px] font-semibold px-1.5 py-0.5 rounded-full bg-amber-50 text-amber-700 border border-amber-200">Pay on Delivery</span>
                                    @endif
                            ...
                            <div class="px-4 py-2.5 bg-neutral-50 border-t border-neutral-100 flex flex-wrap items-center justify-between gap-2">
                                <div class="flex items-center gap-3">
                                    <a href="{{ route('account.orders.show', $order) }}" class="text-xs font-semibold text-primary-600 hover:text-primary-700 inline-flex items-center gap-1">
                                        View Details
                    ...
                    @empty
                        <div class="bg-white rounded-xl border border-neutral-200 p-12 text-center">
                            ...
                            <a href="{{ route('products.index') }}" class="inline-flex items-center gap-2 bg-accent-500 hover:bg-accent-600 text-white text-sm font-semibold px-3 py-1.5 rounded-lg transition-colors">
                                Browse Products
                            </a>
                        </div>
                    @endforelse
