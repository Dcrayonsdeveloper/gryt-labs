            ...
            <div class="flex flex-col lg:flex-row gap-8 mt-4">
                ...
                <div class="flex-1">
                    ...
                    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6">
                        ...
                        @php
                            $statusColors = [
                                'confirmed' => 'bg-primary-600/5 text-primary-700 border-primary-600/30',
                                'processing' => 'bg-primary-600/5 text-primary-700 border-primary-600/30',
                                'packed' => 'bg-primary-600/10 text-primary-700 border-primary-600/30',
                                'shipped' => 'bg-primary-600/15 text-[#15383c] border-primary-600/40',
                                'out_for_delivery' => 'bg-primary-600/5 text-primary-700 border-primary-600/30',
                                    ...
                                    @foreach($order->items as $item)
                                        <div class="flex items-center justify-between px-4 py-2 rounded-lg" style="border: 2px solid rgb(240, 240, 240);">
                                            ...
                                            <div class="text-right flex flex-col shrink-0 ml-3">
                                                ...
                                                <span class="text-sm text-[#0F172A] font-semibold">@price($item->total)</span>
                                            </div>
                                        </div>
                                    @endforeach
                                </div>
                            ...
                            <div class="bg-white rounded-xl border border-neutral-100 p-4">
                                ...
                                <dl class="space-y-2.5">
                                    ...
                                    @php
                                        ...
                                        $balanceDue = max(0, (float) $order->total - $paidOnline);
                                        $isPartialPay = $balanceDue > 0 && $paidOnline > 0;
                                    @endphp
                                    <div class="flex justify-between text-[13px]">
                                        <dt class="text-neutral-600">Payment</dt>
                                        <dd class="font-medium text-neutral-700">
                                            @if($isPartialPay)
                                                Partial Pay (Pay on Delivery)
                                            @elseif($paymentMethod === 'cod' && $order->payment_status !== 'paid')
                                                Pay on Delivery
                                            @elseif(in_array($paymentMethod, ['razorpay', 'upi', 'card']))
                                                Online Payment
                                            @else
                                    ...
                                    @if($paidOnline > 0)
                                        <div class="flex justify-between text-[13px]">
                                            <dt class="text-neutral-600">Paid Online</dt>
                                            <dd class="font-semibold text-emerald-700">@price($paidOnline)</dd>
                                        </div>
                                    @endif
                                    @if($isPartialPay)
                                        <div class="flex justify-between text-[13px]">
                                            <dt class="text-neutral-600">Due on Delivery</dt>
                                            <dd class="font-semibold text-amber-700">@price($balanceDue)</dd>
                                        </div>
                                    @elseif($paymentMethod === 'cod' && $order->payment_status !== 'paid' && $paidOnline <= 0)
                                        <div class="flex justify-between text-[13px]">
                                            <dt class="text-neutral-600">Pay on Delivery</dt>
                                            <dd class="font-semibold text-amber-700">@price($order->total)</dd>
                                        </div>
                                    @endif
                                    <div class="flex justify-between items-center text-[13px]">
                                        ...
                                        <dd>
                                            @php
                                                $payStatusLabel = $isPartialPay ? 'Partial' : ucfirst($order->payment_status);
                                                $payStatusClass = $order->payment_status === 'paid' && !$isPartialPay ? 'bg-emerald-50 text-emerald-700' : ($order->payment_status === 'failed' ? 'bg-red-50 text-red-700' : 'bg-amber-50 text-amber-700');
                                                $payDotClass = $order->payment_status === 'paid' && !$isPartialPay ? 'bg-emerald-500' : ($order->payment_status === 'failed' ? 'bg-red-500' : 'bg-amber-500');
                                            @endphp
                                            <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[11px] font-medium {{ $payStatusClass }}">
                                                <span class="w-1.5 h-1.5 rounded-full {{ $payDotClass }}"></span>
                                                {{ $payStatusLabel }}
                                            </span>
                                        </dd>
                                    </div>
                                ...
                                <div class="bg-white rounded-xl border border-neutral-100 p-4">
                                    ...
                                    <div class="space-y-2">
                                        <div class="flex items-center gap-2.5">
                                            <div class="w-8 h-8 rounded-full bg-primary-600/10 flex items-center justify-center">
                            ...
                            @if($order->payment_collected && ($order->metadata['payment_method'] ?? 'cod') === 'cod')
                                <div class="bg-emerald-50 rounded-xl border border-emerald-100 p-4">
                                    ...
                                    <p class="text-[12px] text-emerald-600 mt-1">
                                        Payment of @price($order->total) has been collected.
