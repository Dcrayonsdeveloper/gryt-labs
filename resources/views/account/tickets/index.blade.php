        ...
        <div class="flex flex-col lg:flex-row gap-6">
            ...
            <div class="flex-1 min-w-0">
                <div class="flex items-center justify-between mb-5">
                    ...
                    <a href="{{ route('account.tickets.create') }}" class="inline-flex items-center gap-1.5 px-4 py-2 bg-accent-500 text-white text-sm font-semibold rounded-lg hover:bg-accent-600 transition-colors">
                ...
                <div class="flex gap-2 mb-4">
                    <a href="{{ route('account.tickets.index') }}"
                       class="px-3 py-1.5 rounded-full text-xs font-medium {{ !request('status') ? 'bg-primary-600/10 text-primary-700' : 'bg-neutral-100 text-neutral-600 hover:bg-neutral-200' }} transition-colors">
                        All
                    </a>
                    <a href="{{ route('account.tickets.index', ['status' => 'open']) }}"
                       class="px-3 py-1.5 rounded-full text-xs font-medium {{ request('status') === 'open' ? 'bg-primary-600/10 text-primary-700' : 'bg-neutral-100 text-neutral-600 hover:bg-neutral-200' }} transition-colors">
                        Open
                    </a>
                    <a href="{{ route('account.tickets.index', ['status' => 'answered']) }}"
                       class="px-3 py-1.5 rounded-full text-xs font-medium {{ request('status') === 'answered' ? 'bg-primary-600/10 text-primary-700' : 'bg-neutral-100 text-neutral-600 hover:bg-neutral-200' }} transition-colors">
                        Answered
                    </a>
                    <a href="{{ route('account.tickets.index', ['status' => 'closed']) }}"
                       class="px-3 py-1.5 rounded-full text-xs font-medium {{ request('status') === 'closed' ? 'bg-primary-600/10 text-primary-700' : 'bg-neutral-100 text-neutral-600 hover:bg-neutral-200' }} transition-colors">
                        Closed
                    </a>
                </div>
                ...
                <div class="space-y-3">
                    @forelse($tickets as $ticket)
                        <a href="{{ route('account.tickets.show', $ticket) }}"
                           class="block bg-white border border-neutral-100 rounded-xl p-4 hover:border-primary-600/30 hover:shadow-sm transition-all">
                    ...
                    @empty
                        <div class="bg-white border border-neutral-100 rounded-xl p-8 text-center">
                            ...
                            <a href="{{ route('account.tickets.create') }}" class="text-sm text-primary-600 font-medium hover:underline">Raise your first ticket</a>
                        </div>
                    @endforelse
                </div>
