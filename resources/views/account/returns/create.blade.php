            ...
            <div class="flex flex-col lg:flex-row gap-8">
                ...
                <div class="flex-1">
                    ...
                    <div class="flex items-center gap-2 text-sm text-neutral-600 mb-5">
                        <a href="{{ route('account.returns.index') }}" class="hover:text-primary-600 transition-colors">My Returns</a>
                    ...
                    @if($orders->isEmpty())
                        <div class="bg-white rounded-xl border border-neutral-200 p-12 text-center">
                            ...
                            <a href="{{ route('account.orders.index') }}" class="inline-flex items-center gap-2 bg-accent-500 hover:bg-accent-600 text-white text-sm font-semibold px-3 py-1.5 rounded-lg transition-colors">
                                View Orders
                            </a>
                        </div>
                    @else
                        <form action="{{ route('account.returns.store') }}" method="POST"
                            ...
                            <div class="bg-white rounded-xl border border-neutral-200 overflow-hidden">
                                <div class="px-5 py-3 border-b border-neutral-100 flex items-center gap-2">
                                    <span class="w-5 h-5 bg-accent-500 text-white text-[11px] font-bold rounded-full flex items-center justify-center">1</span>
                                    ...
                                    <div class="grid gap-2">
                                        @foreach($orders as $order)
                                            <label @click="selectedOrder = '{{ $order->id }}'; selectedItems = []"
                                                   :class="selectedOrder == '{{ $order->id }}' ? 'border-primary-600/50 bg-primary-600/5 ring-1 ring-primary-600/30' : 'border-neutral-200 hover:border-neutral-300 bg-white'"
                                                ...
                                                <div class="flex items-center gap-3">
                                                    <div :class="selectedOrder == '{{ $order->id }}' ? 'border-primary-600 bg-accent-500' : 'border-neutral-300'"
                            ...
                            <div class="bg-white rounded-xl border border-neutral-200 overflow-hidden">
                                <div class="px-5 py-3 border-b border-neutral-100 flex items-center gap-2">
                                    <span class="w-5 h-5 bg-accent-500 text-white text-[11px] font-bold rounded-full flex items-center justify-center">2</span>
                            ...
                            <div class="bg-white rounded-xl border border-neutral-200 overflow-hidden">
                                <div class="px-5 py-3 border-b border-neutral-100 flex items-center gap-2">
                                    <span class="w-5 h-5 bg-accent-500 text-white text-[11px] font-bold rounded-full flex items-center justify-center">3</span>
                                ...
                                <div class="p-5 space-y-3">
                                    <div>
                                        ...
                                        <select name="reason_category" required class="w-full rounded-lg border border-neutral-200 text-sm px-3 py-2.5 focus:border-primary-600/50 focus:ring focus:ring-primary-600/15 focus:ring-opacity-50">
                                            <option value="">Select a reason...</option>
                                            <option value="Product not as described" {{ old('reason_category') === 'Product not as described' ? 'selected' : '' }}>Product not as described</option>
                                            <option value="Received damaged/defective item" {{ old('reason_category') === 'Received damaged/defective item' ? 'selected' : '' }}>Received damaged/defective item</option>
                                            <option value="Wrong item received" {{ old('reason_category') === 'Wrong item received' ? 'selected' : '' }}>Wrong item received</option>
                                            <option value="Changed my mind" {{ old('reason_category') === 'Changed my mind' ? 'selected' : '' }}>Changed my mind</option>
                                            <option value="Better price available" {{ old('reason_category') === 'Better price available' ? 'selected' : '' }}>Better price available</option>
                                    ...
                                    <div>
                                        ...
                                        <textarea name="reason" rows="2" placeholder="Tell us more about why you want to return this item..."
                                                  class="w-full rounded-lg border border-neutral-200 text-sm px-3 py-2.5 focus:border-primary-600/50 focus:ring focus:ring-primary-600/15 focus:ring-opacity-50 resize-none">{{ old('reason') }}</textarea>
                                        @error('reason')
                                            <p class="text-sm text-red-600">{{ $message }}</p>
                                        @enderror
                                    </div>
                            ...
                            <div class="bg-white rounded-xl border border-neutral-200 overflow-hidden" x-show="currentOrder" x-cloak>
                                <div class="px-5 py-3 border-b border-neutral-100 flex items-center gap-2">
                                    <span class="w-5 h-5 bg-accent-500 text-white text-[11px] font-bold rounded-full flex items-center justify-center">4</span>
                                ...
                                <div class="p-5">
                                    ...
                                    <div class="space-y-2">
                                        <template x-for="(item, idx) in currentOrder?.items || []" :key="item.id">
                                            <div :class="isSelected(item.id) ? 'border-primary-600/50 bg-primary-600/5/30 ring-1 ring-primary-600/30' : 'border-neutral-200'"
                                                ...
                                                <div @click="toggleItem(item.id)" class="flex items-center gap-3 p-3.5 cursor-pointer">
                                                    <div :class="isSelected(item.id) ? 'bg-accent-500 border-primary-600' : 'border-neutral-300'"
                                                    ...
                                                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
                                                        ...
                                                        <div>
                                                            ...
                                                            <input type="number"
                                                                   ...
                                                                   class="w-full rounded-lg border border-neutral-200 text-sm px-3 py-2 focus:border-primary-600/50 focus:ring focus:ring-primary-600/15 focus:ring-opacity-50">
                                                        </div>
                            ...
                            <div class="flex items-center gap-3 pt-2">
                                <button type="submit"
                                        ...
                                        :class="selectedItems.length === 0 ? 'opacity-50 cursor-not-allowed' : 'hover:bg-accent-600'"
                                        class="inline-flex items-center gap-2 bg-accent-500 text-white text-sm font-semibold px-4 py-2 rounded-lg transition-colors">
