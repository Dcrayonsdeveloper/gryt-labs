        ...
        <div class="container mx-auto px-4 py-8">
            <div class="flex flex-col lg:flex-row gap-8">
                ...
                <div class="flex-1">
                    ...
                    <div class="bg-white rounded-xl border border-neutral-200 mb-4 overflow-hidden">
                        <div class="px-5 py-4 border-b border-neutral-100 flex flex-wrap items-start justify-between gap-3">
                            ...
                            @php
                                $statusColors = [
                                    ...
                                    'pickup_scheduled' => 'bg-primary-600/5 text-primary-700 border-primary-600/30',
                                    'picked_up' => 'bg-primary-600/15 text-[#15383c] border-primary-600/40',
                                    'received' => 'bg-primary-600/5 text-primary-700 border-primary-600/30',
                                ...
                                @foreach($steps as $i => $step)
                                    <div class="flex items-center gap-1 {{ $i < count($steps) - 1 ? 'flex-1' : '' }}">
                                        <div class="flex flex-col items-center">
                                            @if($i <= $currentIndex)
                                                <div class="w-6 h-6 rounded-full {{ $step === 'rejected' ? 'bg-red-500' : 'bg-accent-500' }} flex items-center justify-center">
                                                    @if($step === 'rejected')
                                        ...
                                        @if($i < count($steps) - 1)
                                            <div class="flex-1 h-0.5 {{ $i < $currentIndex ? ($step === 'rejected' ? 'bg-red-300' : 'bg-primary-600') : 'bg-neutral-200' }} rounded-full mx-1 mb-4 sm:mb-0"></div>
                                        @endif
                                    </div>
                                @endforeach
                            </div>
                        ...
                        <div class="grid grid-cols-2 sm:grid-cols-4 divide-x divide-neutral-100">
                            <div class="px-5 py-3.5">
                                ...
                                <a href="{{ route('account.orders.show', $return->order) }}" class="text-sm font-semibold text-primary-600 hover:text-primary-700">
                    ...
                    {{-- Reason --}}
                    @if($return->reason_category || $return->reason || $return->description)
                        <div class="bg-white rounded-xl border border-neutral-200 mb-4 overflow-hidden">
                            <div class="px-5 py-3 border-b border-neutral-100">
                                <h2 class="text-sm font-bold text-neutral-900">Reason</h2>
                            </div>
                            <div class="px-5 py-3.5">
                                <p class="text-sm font-medium text-neutral-800">{{ $return->reason_category ?? $return->reason }}</p>
                                @if($return->reason && $return->reason_category)
                                    <p class="text-sm text-neutral-600 mt-1">{{ $return->reason }}</p>
                                @endif
                                ...
                                <div class="flex items-center gap-3">
                                    ...
                                    <div class="flex-1">
                                        ...
                                        @if($return->pickupPartner->phone)
                                            <a href="tel:{{ $return->pickupPartner->phone }}" class="text-xs text-primary-600 hover:text-primary-700">{{ $return->pickupPartner->phone }}</a>
                                        @endif
                                    </div>
                                </div>
