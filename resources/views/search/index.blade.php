<x-layouts.app>
    ...
    <div class="container mx-auto px-4 py-8">
        ...
        <div class="relative max-w-xl mb-6"
            ...
            <form action="{{ route('search') }}" method="GET" class="relative flex items-center">
                ...
                <input type="text"
                       ...
                       class="w-full pl-10 pr-20 py-3 text-base bg-white border border-neutral-200 rounded-lg focus:outline-none focus:border-primary-600 focus:ring-1 focus:ring-primary-600"
                ...
                {{-- Submit button --}}
                <button type="submit" class="absolute right-3 p-1.5 text-neutral-600 hover:text-primary-600 transition-colors" aria-label="Search">
        ...
        @if($query)
            <p class="text-sm text-neutral-600 mb-6">
                @if($products->total() > 0)
                    {{ number_format($products->total()) }} {{ $products->total() === 1 ? 'result' : 'results' }} for <span class="font-semibold text-neutral-800">"{{ $query }}"</span>
                @else
            ...
            <div class="flex flex-col lg:flex-row gap-8">
                ...
                <div class="flex-1">
                    @if($products->count())
                        ...
                        }" x-init="$nextTick(() => { new IntersectionObserver((e) => { if (e[0].isIntersecting) $data.loadMore(); }, { rootMargin: '200px' }).observe($refs.sentinel); })">
    ...
    @if(!empty($query) && !empty($fbEventId) && $theme->get('facebook_pixel_id', ''))
    <script>
