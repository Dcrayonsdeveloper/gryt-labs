<x-layouts.app>
                    ...
                    @foreach($categories as $cat)
                        <a href="{{ route('category.show', $cat) }}" class="group block rounded-xl overflow-hidden border border-neutral-200 hover:border-primary-400 hover:shadow-md transition-all bg-white">
                            <div class="bg-[#f8f6f3] overflow-hidden" style="height:160px;">
                                ...
                                @endphp
                                @if($catImg)
                                    <img src="{{ $catImg }}" alt="{{ $cat->name }}" class="w-full h-full object-contain group-hover:scale-105 transition-transform duration-300" width="300" height="160" loading="lazy" decoding="async">
                                @else
                    ...
                    @foreach($carouselCategories as $cat)
                        <a href="{{ route('category.show', $cat) }}"
                           ...
                           style="min-width: 120px; max-width: 130px;">
                            ...
                            <span class="text-xs font-medium text-[#0F1111] text-center leading-tight line-clamp-2 group-hover:text-primary-600 transition-colors">
                                {{ $cat->name }}
                            </span>
                        </a>
                    @endforeach
    ...
    @if(isset($sections['product_banner_1']) && $sections['product_banner_1']->is_active && $sections['product_banner_1']->image_url)
        <section class="bg-white">
            <a href="{{ $sections['product_banner_1']->button_link ?? route('products.index') }}" class="block">
                <img src="{{ asset('storage/' . $sections['product_banner_1']->image_url) }}" alt="{{ $sections['product_banner_1']->title }}" class="w-full h-auto object-cover" loading="lazy" decoding="async">
            </a>
        </section>
    @endif
    ...
    @if(isset($sections['product_banner_2']) && $sections['product_banner_2']->is_active && $sections['product_banner_2']->image_url)
        <section class="bg-white">
            <a href="{{ $sections['product_banner_2']->button_link ?? route('products.index') }}" class="block">
                <img src="{{ asset('storage/' . $sections['product_banner_2']->image_url) }}" alt="{{ $sections['product_banner_2']->title }}" class="w-full h-auto object-cover" loading="lazy" decoding="async">
            </a>
        </section>
    @endif
    ...
    function submitConsultation(){
        ...
        fetch('/contact',{method:'POST',headers:{'Content-Type':'application/json','X-CSRF-TOKEN':'{{ csrf_token() }}','Accept':'application/json'},body:JSON.stringify({name:n,phone:p,email:e||'',subject:'Consultation: '+t,message:msg})})
