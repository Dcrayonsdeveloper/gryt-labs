@php
    $store = \App\Models\Setting::get('store_name', config('app.name'));
...
<head>
    ...
    <title>Influencer Login — {{ $store }}</title>
    ...
    <div class="min-h-screen grid lg:grid-cols-2">
        ...
        <div class="hidden lg:flex flex-col justify-between p-12 text-white relative overflow-hidden"
            ...
            <div class="relative z-10 max-w-md">
                <p class="text-xs uppercase tracking-widest text-white/40 mb-3">Influencer Portal</p>
                <h2 class="text-4xl font-extrabold leading-tight">Your coupon.<br>Your numbers.<br><span class="text-emerald-400">In real time.</span></h2>
                <p class="mt-5 text-white/70 leading-relaxed">Track every order placed with your code — sales, discounts and customers, all in one clean dashboard.</p>
                <ul class="mt-8 space-y-3 text-sm text-white/80">
                    <li class="flex items-center gap-2.5"><span class="inline-flex w-5 h-5 rounded-full bg-emerald-500/20 text-emerald-400 items-center justify-center">✓</span> Live orders &amp; revenue</li>
                    <li class="flex items-center gap-2.5"><span class="inline-flex w-5 h-5 rounded-full bg-emerald-500/20 text-emerald-400 items-center justify-center">✓</span> Daily &amp; monthly charts</li>
                    <li class="flex items-center gap-2.5"><span class="inline-flex w-5 h-5 rounded-full bg-emerald-500/20 text-emerald-400 items-center justify-center">✓</span> Export to CSV anytime</li>
                </ul>
            </div>
        ...
        <div class="flex items-center justify-center p-6 sm:p-10">
            <div class="w-full max-w-sm">
                <div class="text-center mb-8">
                    @if($logo)
                        <img src="{{ asset($logo) }}" alt="{{ $store }}" class="h-10 w-auto mx-auto" style="max-height:44px">
                    @else
                        <span class="text-2xl font-extrabold tracking-tight text-neutral-900">{{ $store }}</span>
                    @endif
                    <p class="text-xs text-neutral-400 mt-1.5 uppercase tracking-widest">Influencer Dashboard</p>
                </div>

                <h1 class="text-2xl font-bold text-neutral-900">Welcome back</h1>
                <p class="text-sm text-neutral-500 mb-6">Sign in to view your coupon performance.</p>
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="text" name="username" id="username" value="{{ old('username') }}" required autofocus
                                   autocomplete="username" placeholder="your-username" class="form-input w-full pl-10">
                        </div>
                    </div>
                    ...
                    <div x-data="{ show: false }">
                        ...
                        <div class="relative">
                            ...
                            <input :type="show ? 'text' : 'password'" name="password" id="password" required
                                   autocomplete="current-password" placeholder="••••••••" class="form-input w-full pl-10 pr-10">
                ...
                <p class="text-center text-xs text-neutral-400 mt-8">Need an account? Contact the {{ $store }} team.</p>
            </div>
        </div>
    </div>
</body>
