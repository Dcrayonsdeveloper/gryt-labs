<x-layouts.app>
    ...
    <div class="container mx-auto px-4 py-8 sm:py-12">
        <div class="max-w-3xl mx-auto">
            ...
            @if(!in_array($order->status, ['cancelled', 'returned']))
                @php
                    $canCancel = in_array($order->status, ['pending', 'confirmed', 'processing']);
                    $returnWindowDays = (int) \App\Models\Setting::get('return_window_days', 7);
                    $returnMinHours   = (int) \App\Models\Setting::get('return_min_hours', 24);
                ...
                @if($canCancel || $canReturn)
                    <div class="bg-white border border-neutral-100 rounded-xl p-5 space-y-3">
                        <h2 class="text-[14px] font-semibold text-neutral-900">Manage Order</h2>

                        @if(session('success'))
                            <div class="bg-emerald-50 border border-emerald-200 text-emerald-700 rounded-lg px-4 py-3 text-[13px]">{{ session('success') }}</div>
                        @endif
                        @if(session('error'))
                            <div class="bg-red-50 border border-red-200 text-red-700 rounded-lg px-4 py-3 text-[13px]">{{ session('error') }}</div>
                        @endif

                        @if($canCancel)
                            <form action="{{ route('track-order.cancel', $order) }}" method="POST"
