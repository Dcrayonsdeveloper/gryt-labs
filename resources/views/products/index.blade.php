<x-layouts.app>
    @php $storeName = $theme->get('store_name', config('app.name')); @endphp
    <x-slot name="title">{{ request('category') ? ($categories->firstWhere('slug', request('category'))?->name ?? 'Products') : 'All Products' }} — {{ $storeName }}</x-slot>
    ...
    @push('meta')
        ...
        <meta property="og:title" content="{{ $metaCat ?? ($metaBrand ?? 'All Products') }} — {{ $storeName }}">
        <meta property="og:description" content="{{ $metaDesc }}">
        <meta property="og:type" content="website">
        <meta property="og:url" content="{{ url()->current() }}">
        <meta name="twitter:card" content="summary_large_image">
        <meta name="twitter:title" content="{{ $metaCat ?? ($metaBrand ?? 'All Products') }} — {{ $storeName }}">
    ...
    <div class="bg-primary-600">
        <div class="container mx-auto px-4 py-6 md:py-8">
            ...
            <p class="text-white/70 text-xs mt-2">{{ $products->total() }} products</p>
        </div>
    </div>
    ...
    <div class="container mx-auto px-4 py-6">
        ...
        @if(request()->hasAny(['category', 'brand', 'min_price', 'max_price', 'rating', 'in_stock', 'on_sale']))
            <div class="flex flex-wrap items-center gap-2 mb-5">
                ...
                @endif
                @if(request('brand'))
                    @php $allBrands = (array) request('brand'); @endphp
                    @foreach($allBrands as $brandSlug)
                        @php
                ...
                @endif
                <a href="{{ route('products.index') }}" class="text-xs text-neutral-600 hover:text-primary-600 underline ml-1">Clear all</a>
            </div>
        @endif
            ...
            <div class="flex-1 min-w-0">
                ...
                @else
                    <div class="text-center py-20">
                        ...
                        <a href="{{ route('products.index') }}" class="inline-flex items-center gap-2 px-4 py-1.5 bg-accent-500 hover:bg-accent-600 text-white text-sm font-semibold rounded-lg transition-colors">
