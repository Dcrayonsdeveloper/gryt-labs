<x-layouts.app>
    ...
    @php
        $pdpTwoCol = $theme->get('pdp_layout') === 'two-column';
        $certImg = $theme->get('product_certifications_image', '');
    @endphp
    <div class="container mx-auto px-4 py-4 lg:py-6">
        <!-- Product Main Section -->
        <div class="grid grid-cols-1 {{ $pdpTwoCol ? 'lg:grid-cols-2' : 'lg:grid-cols-12' }} gap-4 lg:gap-6" id="pdp-main-grid">
            <!-- Image Gallery — Col 1 -->
            <div x-data="{
                 ...
                 class="{{ $pdpTwoCol ? '' : 'lg:col-span-5' }}" id="pdp-image-col">
            ...
            <div class="{{ $pdpTwoCol ? '' : 'lg:col-span-7' }} space-y-3">
                <!-- Brand -->
                @if($product->brand)
                ...
                <!-- Available Coupons -->
                @if($theme->get('show_product_coupons', '1') === '1' && isset($availableCoupons) && $availableCoupons->count())
                <div class="space-y-2">
                    ...
                    if ($packsEnabled) {
                        ...
                        foreach ($tiers as $tier) {
                            ...
                            $packsData[] = [
                                ...
                                'mrp' => $packMrp,
                                'savings' => $savings,
                                'savingsPct' => $packMrp > 0 ? (int) round(($savings / $packMrp) * 100) : 0,
                            ];
                        }
                    }
                @endphp
                ...
                <div x-data x-init="$store.pdpPack.init({{ \Illuminate\Support\Js::from($packsData) }}, 1)" class="mb-3">
                        ...
                        <template x-for="pack in $store.pdpPack.packs" :key="pack.qty">
                            <label @click="$store.pdpPack.select(pack.qty)"
                                ...
                                <div class="flex-1 min-w-0">
                                    <div class="flex items-center gap-2 flex-wrap">
                                        ...
                                        <template x-if="pack.savings > 0">
                                            <span class="text-[10px] font-bold text-white bg-green-700 px-1.5 py-0.5 rounded" x-text="'SAVE ' + $store.pdpPack.formatPrice(pack.savings)"></span>
                                        </template>
                                ...
                                <div class="text-right shrink-0">
                                    ...
                                    <template x-if="pack.savingsPct > 0">
                                        <span class="text-[10px] font-semibold text-green-700" x-text="'Save ' + pack.savingsPct + '%'"></span>
                                    </template>
                                </div>
                            </label>
                        </template>
                ...
                {{-- Buy Box (inline) --}}
                <div x-data="quantitySelector()" class="mt-4 border-t border-[#E3E6E6] pt-4 space-y-3">
                    ...
                    <div class="flex gap-2 sm:gap-3" data-sticky-trigger>
                        @if($product->stock_quantity > 0)
                            ...
                            <button x-data="{ buying: false }"
                                    @click="if(buying) return; buying = true; const _qty = {{ $useStorePackQty ? '$store.pdpPack.currentQty' : 'quantity' }};
                                    fetch('/api/shiprocket-checkout-token', { method: 'POST', headers: { 'Content-Type': 'application/json', 'Accept': 'application/json' }, body: JSON.stringify({ product_id: {{ $product->id }}, quantity: _qty }) }).then(r => r.json()).then(data => { if (data.success && data.token && typeof window.shiprocketCheckoutCustomHandler === 'function') { window.shiprocketCheckoutCustomHandler({ token: data.token, fallbackUrl: '{{ route('checkout.index') }}', onInitiationFailure: function() { window.location.href = '{{ route('checkout.index') }}'; } }); } else if (data.success && data.token && typeof HeadlessCheckout !== 'undefined') { HeadlessCheckout.addToCart($event, data.token, { fallbackUrl: '{{ route('checkout.index') }}', isInitiatedFromApp: true }); } else { axios.post('/cart/add', { product_id: {{ $product->id }}, quantity: _qty }).then(() => { window.location.href = '{{ route('checkout.index') }}'; }); } buying = false; }).catch(() => { axios.post('/cart/add', { product_id: {{ $product->id }}, quantity: _qty }).then(() => { window.location.href = '{{ route('checkout.index') }}'; }); buying = false; })"
                                    :disabled="buying"
                                    class="flex-1 flex items-center justify-center gap-2 bg-[#FFD814] hover:bg-[#F7CA00] text-[#0F1111] font-semibold py-2.5 px-3 rounded-lg text-sm transition-colors shadow-sm disabled:opacity-70">
                    ...
                    <div class="grid grid-cols-2 sm:grid-cols-{{ min(count($__trustBadges), 4) }} gap-2 sm:gap-1.5 py-2 border-t border-[#E3E6E6]">
        ...
        <section class="mt-8 {{ $pdpTwoCol ? 'border-t border-[#E3E6E6] pt-6' : '' }}" @if($pdpTwoCol) x-data="{ activeTab: '{{ $defaultTab }}' }" @endif>

            @if($pdpTwoCol)
            {{-- Tabbed navigation (Ayurvexa) --}}
            <div class="flex border-b border-[#E3E6E6] gap-0 overflow-x-auto" style="-ms-overflow-style:none;scrollbar-width:none;">
                @if($hasDescription)
                <button @click="activeTab = 'description'" :class="activeTab === 'description' ? 'border-primary-600 text-primary-600 font-semibold' : 'border-transparent text-[#3a3a3a] hover:text-[#0F1111] hover:border-[#C7C8C8]'" class="px-4 py-3 text-sm font-medium border-b-2 whitespace-nowrap transition-colors -mb-px">Product Description</button>
                @endif
                @if($hasAttributes)
                <button @click="activeTab = 'details'" :class="activeTab === 'details' ? 'border-primary-600 text-primary-600 font-semibold' : 'border-transparent text-[#3a3a3a] hover:text-[#0F1111] hover:border-[#C7C8C8]'" class="px-4 py-3 text-sm font-medium border-b-2 whitespace-nowrap transition-colors -mb-px">Product Details</button>
                @endif
                @if($hasInfo)
                <button @click="activeTab = 'information'" :class="activeTab === 'information' ? 'border-primary-600 text-primary-600 font-semibold' : 'border-transparent text-[#3a3a3a] hover:text-[#0F1111] hover:border-[#C7C8C8]'" class="px-4 py-3 text-sm font-medium border-b-2 whitespace-nowrap transition-colors -mb-px">Product Information</button>
                @endif
                <button @click="activeTab = 'faq'" :class="activeTab === 'faq' ? 'border-primary-600 text-primary-600 font-semibold' : 'border-transparent text-[#3a3a3a] hover:text-[#0F1111] hover:border-[#C7C8C8]'" class="px-4 py-3 text-sm font-medium border-b-2 whitespace-nowrap transition-colors -mb-px">FAQ</button>
            </div>
            ...
            {{-- Product Description --}}
            @if($hasDescription)
            <div @if($pdpTwoCol) x-show="activeTab === 'description'" class="pt-6" @else class="border-t border-[#E3E6E6] pt-6 mb-8" @endif>
            ...
            @if($hasAttributes)
            <div @if($pdpTwoCol) x-show="activeTab === 'details'" x-cloak class="pt-6" @else class="border-t border-[#E3E6E6] pt-6 mb-8" @endif>
                @if(!$pdpTwoCol)<h2 class="text-lg font-bold text-[#0F1111] mb-4">Product Details</h2>@endif
            ...
            @if($hasInfo)
            <div @if($pdpTwoCol) x-show="activeTab === 'information'" x-cloak class="pt-6" @else class="border-t border-[#E3E6E6] pt-6 mb-8" @endif>
                @if(!$pdpTwoCol)<h2 class="text-lg font-bold text-[#0F1111] mb-4">Product Information</h2>@endif
        ...
        (function() {
            ...
            document.getElementById('sticky-buy-btn').addEventListener('click', function(evt) {
                if (typeof window.__srBuyNow === 'function') {
                    window.__srBuyNow(evt, this, {{ $product->id }});
                } else {
                    axios.post('/cart/add', {product_id: {{ $product->id }}, quantity: getQty()})
                        .then(function(){ window.location.href = '{{ route("checkout.index") }}'; });
                }
            });
