<x-layouts.admin>
    ...
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h1 class="text-xl font-semibold text-gray-900 leading-tight">Flash sales</h1>
    ...
    @php
        ...
        $now = now();
        $pageIds = $flashSales->pluck('id')->toArray();
    @endphp
    ...
    <div class="bg-white rounded-xl shadow-sm overflow-hidden"
        ...
        <div class="flex items-center justify-between border-b border-gray-200 px-3">
            <div class="flex items-center gap-0">
                ...
                @foreach($tabs as $statusKey => $label)
                    <a href="{{ route('admin.flash-sales.index', array_merge(request()->except('status', 'page'), $statusKey ? ['status' => $statusKey] : [])) }}"
        ...
        <div class="flex items-center gap-2 p-3 border-b border-gray-200">
            <form action="{{ route('admin.flash-sales.index') }}" method="GET" class="flex items-center gap-2 flex-1">
                @if(request('status'))
                ...
                <div class="flex-1 relative">
                    ...
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Filter flash sales"
                ...
                @if(request()->hasAny(['search']))
                    <a href="{{ route('admin.flash-sales.index', request('status') ? ['status' => request('status')] : []) }}"
            ...
            <table class="w-full border-collapse">
                <thead>
                    <tr class="border-b border-gray-200">
                        ...
                        <th class="px-3 py-2 text-left text-xs font-medium text-gray-600">Discount</th>
                        <th class="px-3 py-2 text-left text-xs font-medium text-gray-600">Start</th>
                        <th class="px-3 py-2 text-left text-xs font-medium text-gray-600">End</th>
                        <th class="px-3 py-2 text-left text-xs font-medium text-gray-600">Status</th>
                        <th class="px-3 py-2 text-right text-xs font-medium text-gray-600">Products</th>
                    </tr>
                </thead>
                <tbody>
                    @forelse($flashSales as $sale)
                        <tr class="border-b border-gray-100 cursor-pointer hover:bg-gray-50"
                            x-on:click="if(!$event.target.closest('input[type=checkbox]')) window.location.href='{{ route('admin.flash-sales.edit', $sale) }}'">
                            <td class="w-9 px-2 py-2 pl-3 text-center" x-on:click.stop>
                                <input type="checkbox"
                                       class="w-4 h-4 cursor-pointer accent-gray-900"
                                       :checked="selected.includes({{ $sale->id }})"
                                       x-on:change="toggle({{ $sale->id }})">
                            </td>
                            <td class="px-3 py-2">
                                <span class="text-[13px] font-medium text-gray-900">{{ $sale->name }}</span>
                                @if($sale->description)
                                    <p class="text-xs text-gray-600 mt-0.5 max-w-[250px] overflow-hidden text-ellipsis whitespace-nowrap" title="{{ $sale->description }}">{{ $sale->description }}</p>
                                @endif
                            </td>
                            ...
                            <td class="px-3 py-2 text-[13px] text-gray-800">
                                {{ $sale->starts_at->format('M d, Y H:i') }}
                            </td>
                            <td class="px-3 py-2 text-[13px] text-gray-800">
                                {{ $sale->ends_at->format('M d, Y H:i') }}
                            </td>
                            <td class="px-3 py-2">
                                @if($sale->isActive())
                            ...
                            <td class="px-3 py-2 text-[13px] text-gray-800 text-right">
                                {{ $sale->products_count }}
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="7" class="px-3 py-12 text-center">
                                <div class="flex flex-col items-center">
                                    <svg width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="#c9c9c9" stroke-width="1.5" class="mb-3"><path stroke-linecap="round" stroke-linejoin="round" d="M13 10V3L4 14h7v7l9-11h-7z"/></svg>
                                    <p class="text-sm font-medium text-gray-900 mb-1">No flash sales found</p>
                                    <p class="text-[13px] text-gray-600 mb-4">
                                        @if(request()->hasAny(['search', 'status']))
                                            Try changing the filters or search term.
                                        @else
                                            Create a flash sale to offer limited-time discounts.
                                        @endif
                                    </p>
                                    @if(!request()->hasAny(['search', 'status']))
                                        <a href="{{ route('admin.flash-sales.create') }}"
        ...
        @if($flashSales->total() > 0)
            <div class="flex items-center justify-center gap-4 p-3 border-t border-gray-200">
                <div class="flex items-center gap-2">
                    ...
                    @else
                        <a href="{{ $flashSales->previousPageUrl() }}"
                           class="p-1.5 text-gray-600 hover:bg-gray-100 rounded-md inline-flex no-underline">
                            <svg width="16" height="16" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M12.707 5.293a1 1 0 010 1.414L9.414 10l3.293 3.293a1 1 0 01-1.414 1.414l-4-4a1 1 0 010-1.414l4-4a1 1 0 011.414 0z" clip-rule="evenodd"/></svg>
                        </a>
                    @endif

                    <span class="text-[13px] text-gray-800">
                        {{ $flashSales->firstItem() }}-{{ $flashSales->lastItem() }} of {{ $flashSales->total() }}
                    </span>

                    @if($flashSales->hasMorePages())
                        <a href="{{ $flashSales->nextPageUrl() }}"
                           class="p-1.5 text-gray-600 hover:bg-gray-100 rounded-md inline-flex no-underline">
