@php
    $tplSubject   = \App\Models\Setting::get('abandoned_cart_r3_subject', 'Last chance! 10% off your cart — expiring soon');
    $tplPreheader = \App\Models\Setting::get('abandoned_cart_r3_preheader', 'This is our biggest offer for you');
    $tplHeading   = \App\Models\Setting::get('abandoned_cart_r3_heading', 'Last chance — your cart is expiring!');
    $tplBodyRaw   = \App\Models\Setting::get('abandoned_cart_r3_body', "Hi {name},\n\nThis is your final reminder. Your cart items may not be available much longer. We're giving you our best discount to help you decide.");
    $tplCta       = \App\Models\Setting::get('abandoned_cart_r3_cta', 'Get {discount}% Off — Last Chance!');
...
<tr><td style="background:#CC0C39;padding:20px;text-align:center;">
<h1 style="color:#fff;font-size:20px;margin:0 0 4px;">{{ $tplHeading }}</h1>
<p style="color:rgba(255,255,255,0.8);font-size:13px;margin:0;">{{ $tplPreheader }}</p>
</td></tr>
...
<div style="background:#FFF0F3;border:2px dashed #CC0C39;border-radius:8px;padding:16px;text-align:center;margin:20px 0;">
<p style="margin:0 0 4px;font-size:12px;color:#CC0C39;font-weight:bold;">FINAL OFFER - DON'T MISS OUT</p>
<p style="margin:0;font-size:32px;font-weight:bold;color:#CC0C39;">{{ $discountPct }}% OFF</p>
<p style="margin:4px 0;font-size:16px;font-weight:bold;color:#111;">Code: {{ $discountCode }}</p>
<p style="margin:4px 0 0;font-size:11px;color:#CC0C39;">Valid for 48 hours — then it's gone!</p>
</div>
...
<a href="{{ $cartUrl }}" style="background:#CC0C39;color:#fff;text-decoration:none;padding:14px 40px;border-radius:6px;font-size:15px;font-weight:600;display:inline-block;">{{ $ctaRendered }}</a>
</div>
