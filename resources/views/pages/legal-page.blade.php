<x-layouts.app>
    ...
    @php
        $iconBg = match($page->slug) {
            'privacy-policy'   => 'bg-primary-600/5',
        ...
        $iconColor = match($page->slug) {
            'privacy-policy'   => 'text-primary-600',
            ...
            @if($sections)
                @foreach($sections as $section)
                    <div class="bg-white border border-neutral-100 rounded-xl p-5 sm:p-6 mb-4
                        ...
                        {!! \App\Helpers\HtmlSanitizer::clean($section) !!}
                    </div>
                @endforeach
            @else
                <div class="bg-white border border-neutral-100 rounded-xl p-5 sm:p-6 mb-4">
                    <p class="text-[13px] text-neutral-600 italic text-center">Please contact us at {{ $theme->get('support_email', '') }} for more details about this policy.</p>
                </div>
            @endif
