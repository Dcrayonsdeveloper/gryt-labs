{{-- Newsletter Discount Popup — Centered modal overlay --}}
@guest
@php
...
<div x-data="exitIntent()" x-show="show" x-cloak
    ...
    <div x-transition:enter="transition ease-out duration-300"
        ...
        <div class="px-6 pt-8 pb-6 text-center">
            {{-- Logo --}}
            @if($storeLogo)
                <img src="{{ asset($storeLogo) }}" alt="{{ $storeName }}" class="h-10 w-auto mx-auto mb-5 object-contain">
            @else
                <p class="text-base font-bold text-neutral-800 mb-5">{{ $storeName }}</p>
            @endif

            {{-- Form --}}
            <div x-show="!success">
                <h2 class="text-xl sm:text-2xl font-extrabold text-neutral-900 leading-snug mb-5">"{{ $popupTitle }}"</h2>
                ...
                <form @submit.prevent="submitEmail()" class="flex flex-col gap-3">
                    ...
                    <button type="submit" :disabled="submitting"
                        ...
                        <span x-show="!submitting">{{ $popupButtonText }}</span>
                ...
                <p class="text-[11px] text-neutral-400 mt-3 leading-relaxed">{{ $popupFooterText }}</p>
            </div>
            ...
            <div x-show="success" x-cloak class="py-4">
                ...
                <p class="text-sm text-neutral-500 mt-1">Your {{ $discountPercent }}% off code is on its way.</p>
            </div>
        </div>
    </div>
</div>
