...
>
    ...
    >
        {{-- ── Header ──────────────────────────────────────────────────── --}}
        <div class="px-4 py-3 flex items-center justify-between shrink-0" style="background: #000000;">
            <div class="flex items-center gap-2.5">
                <div class="w-9 h-9 rounded-full flex items-center justify-center shrink-0">
                    <svg class="w-6 h-6" viewBox="0 0 24 24" fill="none">
                        <rect x="3" y="8" width="18" height="12" rx="3" fill="white"/>
                        <circle cx="8.5" cy="14" r="1.8" fill="#000"/>
                        <circle cx="15.5" cy="14" r="1.8" fill="#000"/>
                        <circle cx="8.5" cy="14" r="0.8" fill="white"/>
                        <circle cx="15.5" cy="14" r="0.8" fill="white"/>
                        <path d="M10 17.5c0-.28.45-.5 1-.5h2c.55 0 1 .22 1 .5s-.45.5-1 .5h-2c-.55 0-1-.22-1-.5z" fill="#000"/>
        ...
        <div
            x-ref="messageList"
            class="flex-1 overflow-y-auto p-3 bg-neutral-50/60 flex flex-col"
        >
            {{-- Empty / Welcome state --}}
            <template x-if="messages.length === 0">
                <div class="flex-1 flex flex-col items-center justify-center text-center px-4 py-6">
                    ...
                    <div class="flex flex-wrap gap-1.5 justify-center">
                        <template x-for="chip in quickChips" :key="chip.label">
                            <button
                                ...
                                class="text-[11px] px-3 py-1 rounded-full text-primary-600 bg-primary-600/5 hover:bg-primary-600/10 transition-colors font-medium whitespace-nowrap"
                                x-text="chip.label"
                            ></button>
                        </template>
                    </div>
                                ...
                                <template x-if="msg.products && msg.products.length > 0">
                                    <div class="mt-2 flex gap-2 overflow-x-auto pb-1 scrollbar-none">
                                        <template x-for="product in msg.products" :key="product.id">
                                            <a
                                                :href="product.url"
                                                class="shrink-0 w-[108px] bg-white rounded-xl border border-neutral-100 overflow-hidden hover:shadow-md hover:border-primary-600/30 transition-all block"
                                            >
                                                ...
                                                <div class="p-1.5">
                                                    ...
                                                    <div class="mt-1.5 text-center text-[9px] font-semibold text-primary-600 bg-primary-600/5 rounded-md py-0.5 hover:bg-primary-600 hover:text-white transition-colors">
                                                        View →
                                                    </div>
                                                </div>
                                            </a>
        ...
        <template x-if="messages.length > 0 && !isTyping">
            <div class="px-3 py-2 border-t border-neutral-100 flex gap-1.5 overflow-x-auto scrollbar-none shrink-0 bg-white">
                <template x-for="chip in quickChips" :key="chip.label">
                    <button
                        @click="sendQuickChip(chip.message)"
                        class="shrink-0 text-[10px] px-2.5 py-1 rounded-full border border-neutral-200 text-neutral-600 bg-neutral-50 hover:bg-primary-600/10 hover:border-primary-600/40 hover:text-primary-600 transition-colors whitespace-nowrap"
                        x-text="chip.label"
                    ></button>
                </template>
            </div>
        ...
        <div class="px-3 py-3 border-t border-neutral-100 bg-white shrink-0">
            <div class="flex items-center gap-2">
                <input
                    ...
                    class="flex-1 px-3.5 py-2 text-sm bg-neutral-100 rounded-full border-0 text-neutral-800 placeholder-neutral-400 focus:outline-none focus:ring-2 focus:ring-primary-600/30 disabled:opacity-50 transition-all"
                    autocomplete="off"
                >
                <button
                    @click="sendMessage()"
    ...
    <div class="relative">
        ...
        <button
            ...
            :style="isOpen ? 'background:#525252' : 'background:#000000'"
