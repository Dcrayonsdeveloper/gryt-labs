...
class SellerController extends Controller
{
    ...
    public function approve(Request $request, Seller $seller): RedirectResponse
    {
        ...
        $this->notifySeller($seller, fn ($email) => Mail::to($email)->send(new SellerApproved($seller)));
    ...
    public function reject(Request $request, Seller $seller): RedirectResponse
    {
        ...
        $this->notifySeller($seller, fn ($email) => Mail::to($email)->send(new SellerRejected($seller, $reason)));
    ...
    public function suspend(Request $request, Seller $seller): RedirectResponse
    {
        ...
        $this->notifySeller($seller, fn ($email) => Mail::to($email)->send(new SellerSuspended($seller, $reason)));

        return back()->with('success', 'Seller has been suspended.');
    }

    private function notifySeller(Seller $seller, \Closure $send): void
    {
        $email = $seller->payout_email ?: $seller->user?->email;
        if (! $email) {
            return;
        }
        try {
            ...
            Log::warning('Seller notification email failed', [
                'seller_id' => $seller->id,
                'error' => $e->getMessage(),
            ]);
        }
