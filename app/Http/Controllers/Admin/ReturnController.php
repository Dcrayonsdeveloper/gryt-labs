...
class ReturnController extends Controller
{
    ...
    public function updateStatus(Request $request, OrderReturn $return): RedirectResponse
    {
        ...
        if (!in_array($validated['status'], $allowed, true)) {
            return back()->with('error', "Cannot change return status from \"{$return->status}\" to \"{$validated['status']}\".");
        }
        ...
        // Restore stock for returned items when return is completed
        if ($validated['status'] === 'completed') {
            $return->restoreReturnedStock();
        }

        try {
            ReturnRequested::dispatch($return);
        } catch (\Exception $e) {
            \Illuminate\Support\Facades\Log::error('ReturnRequested event failed', ['return' => $return->id, 'error' => $e->getMessage()]);
        }
    ...
    {
        // Prevent refunds on rejected or already-completed returns
        if ($return->status === 'rejected') {
            return back()->with('error', 'Cannot process refund for a rejected return.');
        }

        if ($return->status === 'completed' && $return->refund_amount > 0) {
            return back()->with('error', 'Refund has already been processed for this return.');
        }
        ...
        $maxRefundable = (float) $return->order->total;
        if ((float) $validated['amount'] > $maxRefundable) {
            return back()->with('error', 'Refund amount cannot exceed order total of ' . format_price($maxRefundable) . '.');
        }
        ...
        // Credit note only for wallet refunds
        $customer = $return->order->user;
        if ($customer && $validated['amount'] > 0 && $validated['refund_method'] === 'wallet') {
            CreditNote::create([
                'user_id' => $customer->id,
                'return_id' => $return->id,
        ...
        try {
            ...
            \Illuminate\Support\Facades\Log::error('RefundProcessed event failed', ['return' => $return->id, 'error' => $e->getMessage()]);
        }

        return back()->with('success', "Refund of " . format_price($validated['amount']) . " processed via {$validated['refund_method']}");
    }
}
