...
class EnquiryController extends Controller
{
    ...
    public function bulkAction(Request $request): RedirectResponse
    {
        ...
        $query = Enquiry::whereIn('id', $ids);

        match ($action) {
            'delete' => $query->delete(),
            'read'   => $query->update(['status' => 'read', 'is_read' => true, 'read_at' => now()]),
            'closed' => $query->update(['status' => 'closed']),
            default  => null,
        };

        return back()->with('success', count($ids) . ' enquiry(s) updated.');
    }
}
