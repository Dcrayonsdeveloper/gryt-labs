...
class BlogPostController extends Controller
{
    ...
    public function bulkAction(Request $request): RedirectResponse
    {
        ...
        $query = BlogPost::whereIn('id', $ids);

        match ($action) {
            'delete'    => $query->delete(),
            'publish'   => $query->update(['is_published' => true, 'published_at' => now()]),
            'unpublish' => $query->update(['is_published' => false, 'published_at' => null]),
            default     => null,
        };

        return back()->with('success', count($ids) . ' post(s) updated.');
    }
}
