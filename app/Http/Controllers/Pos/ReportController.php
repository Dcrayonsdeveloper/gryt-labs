...
class ReportController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $hourly = PosSale::where('store_id', $storeId)
            ->where('status', 'completed')
            ->whereDate('created_at', $today)
            ->selectRaw(DbCompat::hour('created_at') . ' as hour, SUM(total) as total, COUNT(*) as count')
            ->groupByRaw(DbCompat::hour('created_at'))
            ->orderBy('hour')
            ->get()
            ->map(fn ($h) => [
                'hour'  => (int) $h->hour,
    ...
    public function monthly(Request $request): JsonResponse
    {
        ...
        $monthly = PosSale::where('store_id', $storeId)
            ...
            ->selectRaw(DbCompat::month('created_at') . ' as month, COUNT(*) as bills, SUM(total) as revenue, SUM(tax) as tax, SUM(discount) as discount')
            ->groupByRaw(DbCompat::month('created_at'))
            ->orderBy('month')
            ->get()
            ->map(fn ($m) => [
                'month'    => (int) $m->month,
